...
CAMP=$ROOT/crustify/campaigns/scheduler
LOGDIR=$CAMP/logs
...
STATE=$ROOT/.campaign-done/scheduler
mkdir -p $STATE
...
UNITS_TSV=$ROOT/crustify-scheduler/UNITS.tsv
...
count() {
  ...
  #
  PORTED=$ROOT/crates/compiler/deeptools/src
  todos=$(grep -rhE 'todo!' $PORTED --include=*.rs 2>/dev/null \
            | grep -vE '^\s*(//|/\*|\*)' | wc -l | tr -d ' ')

  # ⛔⛔⛔ RULE 1 IS A GATE, NOT A REQUEST: **A TRAIT IS AN IMMEDIATE STOP.**
  # The deleted port declared EIGHT traits in the scheduler (Dsc2Store, Dsc2Reads, Dsc2Tree,
  # Dsc2Stages, Dsc2Sites, Dsc2State, DdcState, DscState) and FIVE in the bridge (ScheduleView, Dsc,
  # AccessRecord, Stage, LoopBodyOp). Each one turned a C++ member access into a method somebody else
  # had to implement, and that somebody was 20,494 lines of schedule/stages/ with no C++ counterpart,
  # holding 130 todo!()s. 199,023 Rust lines were deleted for ~31,000 lines of working C++ because of
  # this and nothing else. So the count is checked EVERY STAGE and a single declaration stops the run:
  # catching it at the next stage boundary is the difference between deleting one file and deleting a
  # week. `#[derive(...)]` and impls of FOREIGN traits (Display, Default, Ord, ...) are untouched —
  # this greps DECLARATIONS only.
  # ⭐ SCOPED TO THE RE-PORTED AREAS, and baselined everywhere else. Six trait declarations SURVIVE the
  # nuke and are legitimately a different kind — they carry no data on another type's behalf:
  #   arch.rs Arch, model.rs Model, workload.rs Workload  — the crate's COMPILE-TIME config
  #     parameters; deeptools/CLAUDE.md's architecture invariants require Model/Arch/quant to flow.
  #   islands/dataflow_ir/link.rs UnitKind: Sealed, and Sealed  — SEALED type-level guards.
  #   islands/sentient/dialects/sentient.rs SyncPeer: UnitKind  — same family.
  # So: ZERO tolerance inside the re-ported directories, and a hard ceiling of 6 crate-wide, which is
  # what catches a data-carrier trait being parked in islands/ to dodge the first check.
  TRAIT_RE='^\s*(pub(\([a-z]+\))? )?(unsafe )?trait [A-Za-z0-9_]+'
  traits=$(grep -rhE "$TRAIT_RE" $PORTED/schedule $PORTED/bridges --include=*.rs 2>/dev/null \
             | grep -vE '^\s*(//|/\*|\*)' | wc -l | tr -d ' ')
  traits_all=$(grep -rhE "$TRAIT_RE" $PORTED --include=*.rs 2>/dev/null \
             | grep -vE '^\s*(//|/\*|\*)' | wc -l | tr -d ' ')
  if [ "${traits:-0}" -gt 0 ] || [ "${traits_all:-0}" -gt 6 ]; then
    say "⛔⛔ TRAIT DECLARED — RULE 1 VIOLATED (re-ported=$traits, crate=$traits_all, baseline 6)."
    say "⛔ This is the mechanism that produced 199,023 Rust lines for ~31,000 lines of C++. STOPPING."
    grep -rnE "$TRAIT_RE" $PORTED --include=*.rs 2>/dev/null \
      | grep -vE '^\s*(//|/\*|\*)' | head -20 >> $TRACE
    exit 10
  fi

  # ⭐ THE RATIO, because it is the symptom that was visible for five days and never read: the C++ in
  # scope is ~31,000 lines (L3DlOpsScheduler.cpp 8,033 + ddc/*.cpp 13,029 + DSC2ToDataflowIR 1,739 +
  # the dsc/ headers and the bodies this campaign actually needs). A faithful port lands near 1x. The
  # deleted one was 6.4x overall and 49x on the bridge. If this climbs past ~2x, the shape is drifting
  # again and it is cheaper to stop and look than to keep porting.
  rlines=$(find $PORTED -name '*.rs' -exec cat {} + 2>/dev/null | wc -l | tr -d ' ')
  ...
  say "ANCHORS: filled=$filled of $NUNITS   todo!=${todos:-0}   traits=${traits:-0} (MUST BE 0)   rust=${rlines:-0} lines vs ~31,000 C++"
...
remainders() {
  grep -hoE '/// Replaces: e[0-9]{3}_[A-Za-z0-9_]+' $HOMES 2>/dev/null \
    ...
    | grep -Fxf <(printf '%s\n' "$NAMES") > /tmp/filled-scheduler.txt
...
say "=== SCHEDULER RE-PORT CAMPAIGN START on $BRANCH at $(git -C $ROOT rev-parse --short HEAD) ==="
count
remainders
