...
pub fn register(d: &mut Dispatch) {
    ...
    d.register(OpKind::ScfIf, LatencyCategory::Zero, scf_if);
    d.register(OpKind::ScfYield, LatencyCategory::Zero, scf_yield);
...
fn scf_if(
    ...
    let region: &[Operation<'static>] = if condition {
        op.regions.first().copied().unwrap_or(&[])
    } else {
        op.regions.get(1).copied().unwrap_or(&[])
    };
...
fn scf_for(
    ...
    // The induction variable is a VALUE the loop binds, so it has to be stated: a
    // spelled default (`"%i"`) used to stand in for an absent one, which quietly
    // bound a name the body might not be reading.
    let iter_var = match op.attr(AttrKey::IterVar) {
        Some(Attr::Ssas(v)) if !v.is_empty() => v[0],
        _ => return Err("scf.for: missing 'iter_var' attribute".into()),
    };

    let body_region: &[Operation<'static>] = op.regions.first().copied().unwrap_or(&[]);

    let iter_arg_names: Vec<Ssa> = match op.attr(AttrKey::IterArgs) {
        Some(Attr::Ssas(v)) => v.to_vec(),
        _ => Vec::new(),
    };
...
fn for_op(
    ...
    while i < upper_bound {
        ...
        ctx.set_value(iter_var, Value::Index(i));
...
struct RegionMapPlan {
    ...
    dies_at: std::collections::HashMap<usize, Vec<Ssa>>,
}
...
fn body_dies_at(
    ops: &[Operation],
    plan: &crate::metal::MapRegionPlan,
) -> std::collections::HashMap<usize, Vec<Ssa>> {
    ...
    let mut defined: HashSet<Ssa> = HashSet::new();
    for op in ops {
        if let Some(r) = op.result {
            defined.insert(r);
        }
        if let Some(crate::ir::Attr::Ssas(names)) = op.attr(AttrKey::ResultNames) {
            defined.extend(names.iter().copied());
        }
    }
    // Highest body index at which each value is used — operands, plus the
    // attributes that NAME values (`iter_var`/`iter_args`/…). Those used to be
    // recognized by a leading `%` on a string, which both over-matched (a literal
    // that happened to start with `%`) and under-matched.
    let mut last_use: HashMap<Ssa, usize> = HashMap::new();
    for (i, op) in ops.iter().enumerate() {
        for &operand in op.operands {
            last_use.insert(operand, i);
        }
        for (_, attr) in op.attributes {
            if let crate::ir::Attr::Ssas(xs) = attr {
                for &x in *xs {
                    last_use.insert(x, i);
                }
            }
    ...
    let mut dies: HashMap<usize, Vec<Ssa>> = HashMap::new();
...
#[cfg(metal)]
fn cached_region_map_plan(ops: &[Operation<'static>]) -> Option<std::rc::Rc<RegionMapPlan>> {
    ...
    CACHE.with(|c| {
        c.borrow_mut()
            ...
            .or_insert_with(|| {
                ...
                if ops.iter().any(|o| o.op_type == OpKind::LinalgMatmul) {
                    return None;
                }
