...
///
/// The operation is `'static` because a BAKED program is: `#[forward]` emits the op
/// stream as const data in the binary, and the runtime values a handler builds from
/// it — a [`MemRef`](crate::memref::MemRef)'s `coordinate_set`, an
/// [`AccessTile`](crate::memref::AccessTile)'s `base_map` — borrow the op's affine
/// data rather than copying it.
pub type HandlerFn =
    fn(&Operation<'static>, &mut CoreContext, &ExecutionEnv) -> Result<Option<Value>, String>;

/// Op-name -> handler table, plus the parallel latency-category table that the
/// Python registry keeps in lockstep.
pub struct Dispatch {
    handlers: FxHashMap<OpKind, HandlerFn>,
    latency: FxHashMap<OpKind, LatencyCategory>,
}
...
impl Dispatch {
    ...
    pub fn register(&mut self, op: OpKind, cat: LatencyCategory, f: HandlerFn) {
        self.handlers.insert(op, f);
        self.latency.insert(op, cat);
    }

    /// Look up a handler — mirrors `dispatch(op_name)`.
    pub fn handler(&self, op: OpKind) -> Option<HandlerFn> {
        self.handlers.get(&op).copied()
    }
