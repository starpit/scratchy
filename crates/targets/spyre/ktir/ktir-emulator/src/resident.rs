...
use crate::interpreter::{Arg, Output, execute_function_in_exec_only};
use crate::ir::{Attr, IRFunction, IRModule, Operation, Ssa, Value};
    ...
    fn walk(ops: &[Operation], best: &mut (usize, usize, usize), nmm: &mut usize) {
        for op in ops {
            ...
            if op.op_type == OpKind::KtdpConstructMemoryView
                && let Some(Attr::IntList(v)) = op.attr(AttrKey::Shape)
                && v.len() == 2
            {
...
fn op_type_histogram(ops: &[Operation]) -> String {
    let mut counts: std::collections::BTreeMap<OpKind, usize> = std::collections::BTreeMap::new();
    fn walk(ops: &[Operation], counts: &mut std::collections::BTreeMap<OpKind, usize>) {
        for op in ops {
            *counts.entry(op.op_type).or_default() += 1;
            for rg in op.regions {
                walk(rg, counts);
            }
        }
...
struct ResidentProgram {
    /// The launch group's functions, keyed by name for the `ProgramSpec`'s node
    /// lookups. The IR itself is `'static` const data; this map only owns the keys.
...
pub struct ResidentExecutor {
    ...
    last_token_segs: std::cell::RefCell<HashMap<usize, Vec<Segment<'static>>>>,
...
impl ResidentExecutor {
    ...
    pub fn new(funcs: &[IRFunction<'static>], spec: &ProgramSpec) -> Result<Self, String> {
        Self::new_multi(vec![(funcs, spec)])
    }
    ...
    pub fn new_native(funcs: &[IRFunction<'static>], spec: &ProgramSpec) -> Result<Self, String> {
        Self::build(vec![(funcs, spec)], true)
    }
    ...
    fn run_fused_decode_attention(
        ...
        // argument -> tensor id, from the node bindings.
        let arg_tid: HashMap<Ssa, u64> = node.bindings.iter().map(|b| (b.arg, b.tensor)).collect();
        let read_arg = |arg: Ssa, n: usize| -> Result<Vec<f32>, String> {
            let tid = *arg_tid
                .get(&arg)
                .ok_or_else(|| format!("fused attn: arg %{} not bound", arg.0))?;
            let s = *self
                .stick
                .get(&tid)
        ...
        // Read only the LIVE context rows. The mask is `[1, cap]` and cheap, but Kc/Vc are
        // `[cap, kv_cols]` each and every row costs an f16->f32 decode out of HBM — at a
        // 4096-deep capacity that is most of this function's work, for rows whose softmax
        // weight is exactly zero. Rows are `[cap, kv_cols]` row-major, so the live prefix
        // is a leading slice.
        let live = ktir_optimizer::head_rewrite::DecodeAttnIsland::live_context(&mask);
        let kc = read_arg(island.kc_arg, live * kv_cols)?;
        let kd = read_arg(island.kd_arg, kv_cols)?;
        let vc = read_arg(island.vc_arg, live * kv_cols)?;
        ...
        let o_tid = *arg_tid
            .get(&island.o_arg)
            .ok_or_else(|| format!("fused attn: output arg %{} not bound", island.o_arg.0))?;
        let o_stick = *self
            .stick
            .get(&o_tid)
    ...
    /// As [`set_source_bytes`](Self::set_source_bytes), but TAKING the buffer so the
    /// verbatim path can MOVE it into HBM instead of copying it. Only the matching-dtype
    /// case can move; a dtype crossing still has to decode, so it defers to the borrowing
    /// form.
    pub fn set_source_bytes_owned(
        &mut self,
        tensor: u64,
        bytes: Vec<u8>,
        dtype: DType,
        ...
        if dtype == self.dtype {
            self.mem
                .hbm
                .borrow_mut()
                .write_bytes_owned(s * STICK_BYTES, bytes);
            Ok(())
        } else {
            self.set_source_bytes(tensor, &bytes, dtype)
        }
    }

    /// As [`set_sources`](Self::set_sources), but TAKING the arguments.
    ///
    /// This is the SESSION-CONSTRUCTION form: the whole checkpoint arrives at once, every
    /// weight is written exactly once at its own stick base at its full length, and the
    /// caller has no further use for the buffers. That is precisely when the bytes can be
    /// moved rather than copied — measured at 2.14s for granite-3.1-2b's 362 tensors,
    /// which is otherwise a second full pass over ~4.7 GB.
    pub fn set_sources_owned(&mut self, args: Vec<(u64, Arg)>) -> Result<(), String> {
        #[cfg(metal)]
        ...
        for (tid, arg) in args {
            ...
            match arg {
                Arg::TensorBytes { data, dtype, .. } => {
                    self.set_source_bytes_owned(tid, data, dtype)?;
                }
                Arg::Tensor { data, .. } => self.set_source(tid, &data)?,
                Arg::TensorBf16 { data, shape } => {
                    let f16 = crate::codec::bf16_to_f16(&data, shape.iter().product());
                    self.set_source_bytes_owned(tid, f16, DType::F16)?;
                }
                Arg::Scalar(_) => return Err("resident: scalar args unsupported".into()),
            }
        }
        #[cfg(metal)]
        crate::metal::retain_resident_weights(
            &self.weight_addr_tid(),
            &self.forward_written,
            &just_set,
        );
        Ok(())
    }

    /// Write many sources at once, keyed by TENSOR ID. Unknown ids (a tensor this
    /// program does not reference) are skipped — the caller can hand the whole
    /// weight set.
    ///
    /// The key used to be a SPELLING of the id (`t<id>` / `%t<id>` / `%t<id>_ptr` /
    /// bare `<id>`) that `tensor_id_of_key` parsed straight back to the number the
    /// caller had formatted a line earlier. Both sides hold the id; the seam
    /// carries it.
    ///
    ...
    /// the one name for it that every program agrees on.
    #[cfg(metal)]
    fn weight_addr_tid(&self) -> HashMap<i64, u64> {
        self.stick
            .iter()
            .map(|(&tid, &s)| (s * STICK_BYTES, tid))
            .collect()
    }
    ...
    pub fn run(&mut self, outputs: &[u64]) -> Result<HashMap<u64, Output>, String> {
        // Whole tensors — this convenience entry has no per-output extent to narrow to.
        let want: Vec<(u64, usize)> = outputs.iter().map(|&t| (t, 0)).collect();
        self.run_program(0, &want)
    }
    ...
    /// ⛔ AN OUTPUT CARRIES HOW MANY ELEMENTS THE CALLER WANTS. Decode and prefill share ONE session
    /// and one tensor id space, so every tensor is resident at the WIDEST program's row count — a
    /// decode step that writes one row still had its outputs decoded at the prefill's `m`. Measured
    /// on granite (prefill m=96): 195,428,640 elements per token for a ~90 K result, and the readback
    /// alone fell 33 ms -> 0.2 ms when the prefill width was 8. `0` means "the whole tensor".
    pub fn run_program(
        &mut self,
        idx: usize,
        outputs: &[(u64, usize)],
        ...
        for (seg_i, seg) in segments.iter().enumerate() {
            ...
            match seg {
                Segment::Fused(fs) => {
                    ...
                    // Name the SEGMENT in any failure: a program is hundreds of segments, and the
                    // op-level error alone (an SSA and a shape) does not say which one.
                    ...
                    .map_err(|e| format!("seg{seg_i} fused @{}: {e}", fs.func.name))?;
                }
                Segment::Native(node) => {
                    ...
                    if seg_prof {
                        native_ops = op_type_histogram(func.operations);
                    }
                    if fused {
            ...
            if diag || seg_prof {
                ...
                match seg {
                    ...
                    Segment::Native(_) => {
                        ...
                        n_native += 1;
                        if seg_prof {
                            // The op histogram matters MORE here than for a fused segment: a native
                            // segment gets no GPU offload, so its cost is whatever ops it runs.
                            prof_rows.push((
                                format!("seg{seg_i:>3} native attn  ops:[{native_ops}]"),
                                dt,
                                0,
                            ));
                        }
        ...
        if diag {
            eprintln!(
                "  [resident-seg-diag] {n_fused} fused {t_fused:.1}ms (GPU GEMM/map) | \
                 ...
                 hbm views {} ({} off-base) | tiles {} ({} rank-skipped)",
                crate::dialects::ktdp::VIEW_HBM_TOTAL.swap(0, std::sync::atomic::Ordering::Relaxed),
                crate::dialects::ktdp::VIEW_HBM_OFF_BASE
                    .swap(0, std::sync::atomic::Ordering::Relaxed),
                crate::dialects::ktdp::TILE_TOTAL.swap(0, std::sync::atomic::Ordering::Relaxed),
                crate::dialects::ktdp::TILE_UNCHECKED.swap(0, std::sync::atomic::Ordering::Relaxed)
            );
        }
        if seg_prof {
            ...
            // Weight-cache tally: proves resident weights (incl. the lm_head N-tiles) are
            // decoded+uploaded ONCE and then served from cache on every later pass. The
            // invariant is `unresolved=0`, NOT `misses=0`: a steady state still misses on
            // every tensor the forward WRITES (the KV cache above all), because those bytes
            // really did change and must be re-read. `unresolved` counts the other kind —
            // an immutable weight we already had and failed to recognise — so a nonzero
            // steady-state value there is the weight-re-upload regression to catch.
            #[cfg(metal)]
            eprintln!(
                "    [weight-cache] hits={} misses={} \
                 dropped: mutated={} unresolved={} (steady-state unresolved should be 0)",
                ...
                crate::metal::WEIGHT_CACHE_DROP_MUTATED
                    .swap(0, std::sync::atomic::Ordering::Relaxed),
                crate::metal::WEIGHT_CACHE_DROP_UNRESOLVED
                    .swap(0, std::sync::atomic::Ordering::Relaxed),
            );
            {
                // Which offloads actually FIRED this forward — a path that is compiled but never
                // reached looks identical to a slow one from the outside.
                let pr = crate::metal::offload_proof();
                eprintln!(
                    "    [offload] matmul_loop gpu={} amx={} | gemm_or_blas={} | map_region={}",
                    pr.matmul_loop_gpu, pr.matmul_loop_amx, pr.gemm_or_blas_gpu, pr.map_region_gpu
                );
                crate::metal::reset_offload_proof();
            }
            eprintln!(
                "    [gemm] {} gemms, mean M {:.1} | operand resolve {:.1}ms | compute {:.1}ms \
                 (gpu {:.1}ms [exec {:.1}ms] / amx {:.1}ms)",
                crate::metal::GEMM_COUNT.load(std::sync::atomic::Ordering::Relaxed),
                crate::metal::GEMM_M_SUM.swap(0, std::sync::atomic::Ordering::Relaxed) as f64
                    / crate::metal::GEMM_COUNT
                        .swap(0, std::sync::atomic::Ordering::Relaxed)
                        .max(1) as f64,
                crate::metal::GEMM_RESOLVE_NS.swap(0, std::sync::atomic::Ordering::Relaxed) as f64
                    / 1e6,
                crate::metal::GEMM_COMPUTE_NS.swap(0, std::sync::atomic::Ordering::Relaxed) as f64
                    / 1e6,
                crate::metal::GEMM_NAX_NS.swap(0, std::sync::atomic::Ordering::Relaxed) as f64
                    / 1e6,
                crate::metal::GEMM_GPU_EXEC_NS.swap(0, std::sync::atomic::Ordering::Relaxed) as f64
                    / 1e6,
                crate::metal::GEMM_AMX_NS.swap(0, std::sync::atomic::Ordering::Relaxed) as f64
                    / 1e6,
            );
        }
        ...
        // resident HBM.
        let want: Vec<(u64, usize)> = if outputs.is_empty() {
        ...
        let read: Vec<(u64, i64, usize, Vec<usize>, DType)> = want
            .iter()
            .map(|&(tid, lim)| {
                self.meta_for(tid).map(|(name, stick, n, shape, dt)| {
                    // Clamp to what was asked for, and shrink the reported shape's LEADING dim to
                    // match so the caller's view of the tensor stays consistent with its bytes.
                    let n2 = if lim == 0 { n } else { n.min(lim) };
                    let shape2 = match shape.split_first() {
                        Some((_, rest)) if !rest.is_empty() && n2 < n => {
                            let per_row: usize = rest.iter().product::<usize>().max(1);
                            let mut v = vec![n2.div_ceil(per_row)];
                            v.extend_from_slice(rest);
                            v
                        }
                        _ => shape,
                    };
                    (name, stick, n2, shape2, dt)
                })
            })
            .collect::<Result<_, _>>()?;
    ...
    fn build_last_token_segments(&self, idx: usize) -> Result<Vec<Segment<'static>>, String> {
        ...
        if let Segment::Fused(fs) = &mut segs[ti] {
            // Pointer arg -> tensor id, to spot the result view's root — carried by
            // the segment rather than parsed back out of the argument's name.
            let arg_to_tensor: HashMap<Ssa, u64> = fs.args.iter().copied().collect();
            fs.func.operations = rewrite_last_token_func(
                crate::arena::Arena::global(),
                fs.func.operations,
                &arg_to_tensor,
                results,
            )?;
    ...
    fn meta_for(&self, tid: u64) -> Result<(u64, i64, usize, Vec<usize>, DType), String> {
        let s = *self
            .stick
            .get(&tid)
...
fn rewrite_last_token_func<'a>(
    a: &'a Arena,
    ops: &'a [Operation<'a>],
    arg_to_tensor: &HashMap<Ssa, u64>,
    results: &std::collections::HashSet<u64>,
) -> Result<&'a [Operation<'a>], String> {
    // Pass 1: classify memory views by the value they define.
    ...
    let mut result_views: std::collections::HashSet<Ssa> = std::collections::HashSet::new();
    let mut view_shape: HashMap<Ssa, Vec<usize>> = HashMap::new();
    fn scan_views(
        ops: &[Operation],
        arg_to_tensor: &HashMap<Ssa, u64>,
        results: &std::collections::HashSet<u64>,
        m: &mut Option<usize>,
        result_views: &mut std::collections::HashSet<Ssa>,
        view_shape: &mut HashMap<Ssa, Vec<usize>>,
    ) {
        for op in ops {
            if op.op_type == OpKind::KtdpConstructMemoryView
                && let (Some(res), Some(root)) = (op.result, op.operands.first())
                && let Some(Attr::IntList(shape)) = op.attr(AttrKey::Shape)
                && shape.len() == 2
            {
                let shp: Vec<usize> = shape.iter().map(|&x| x as usize).collect();
                view_shape.insert(res, shp.clone());
                if let Some(&tid) = arg_to_tensor.get(root)
                    && results.contains(&tid)
                {
                    result_views.insert(res);
                    *m = Some(shp[0]);
                }
            }
            for rg in op.regions {
    ...
    if m <= 1 {
        return Ok(ops); // single row already — nothing to prune
    }
    // Activation views: first dim == m, not a result view (weights have first dim
    // == vocab/hidden ≫ m, so they're excluded by the == m test).
    let act_views: std::collections::HashSet<Ssa> = view_shape
        .iter()
        .filter(|(name, shp)| shp[0] == m && !result_views.contains(*name))
        .map(|(&name, _)| name)
        .collect();

    // ⭐ THE ROW CONSTANT IS MINTED, NOT SPELLED. It used to be the invented NAME
    // `%lt_last_row_<m-1>`, unique only because nothing else happened to spell it.
    // A fresh id above every id the function already binds cannot collide.
    let row_const = Ssa(max_ssa(ops) + 1);

    // Pass 2: rewrite. Shrink activation views to [1, k]; pin the row index to m-1
    // on every access tile over a result OR activation view. The op stream is
    // BORROWED, so the rewrite REBUILDS it into the arena rather than mutating.
    fn rewrite<'a>(
        a: &'a Arena,
        ops: &[Operation<'a>],
        ...
        let mut out: Vec<Operation<'a>> = Vec::with_capacity(ops.len());
        for op in ops {
            let mut nop = *op;
            if op.op_type == OpKind::KtdpConstructMemoryView
                ...
                && shape.len() == 2
            {
                // activation A becomes a single row
                let shrunk = vec![1, shape[1]];
                nop = nop.with_attr(a, AttrKey::Shape, Attr::IntList(a.ints(shrunk)));
            }
            if op.op_type == OpKind::KtdpConstructAccessTile
                && let Some(&view) = op.operands.first()
                && (act_views.contains(&view) || result_views.contains(&view))
                && op.operands.len() >= 2
                && op.operands[1] != row_const
            {
                let mut operands = op.operands.to_vec();
                operands[1] = row_const;
                nop.operands = a.ssa(operands);
                *need_const = true;
            }
            if !op.regions.is_empty() {
                let regions: Vec<&[Operation<'a>]> = op
                    .regions
                    .iter()
                    .map(|rg| {
                        a.ops(rewrite(
    ...
    if need_const {
        let c = Operation::new(a, Some(row_const), OpKind::ArithConstant, &[]).with_attr(
            a,
            AttrKey::Value,
            Attr::Int((m - 1) as i64),
        );
        body.insert(
            0,
            Operation {
                result_type: Some(crate::irtype::IrType::Index),
                ..c
            },
        );
    }
...
/// The highest SSA id any op in `ops` defines or reads, recursing into regions —
/// the floor a rewrite mints fresh values above.
fn max_ssa(ops: &[Operation]) -> u32 {
    let mut hi = 0u32;
    for op in ops {
...
fn largest_grid(module: &IRModule<'static>, segments: &[Segment<'static>]) -> usize {
    let mut n = 1usize;
    for seg in segments {
...
///
/// `args` and `outputs` are keyed by TENSOR ID (`outputs` empty = the program's
/// declared results); the returned map is keyed the same way.
pub fn execute_resident(
    ...
    let mut exec = ResidentExecutor::new(funcs, spec)?;
