...
use crate::opkind::{Dialect, OpKind};
...
/// Lower `f` to an MSL kernel string. Errors if the function isn't the
/// supported element-wise shape (with a message pointing at what tripped it).
pub fn emit_msl(f: &IRFunction) -> Result<String, String> {
    Ok(emit_kernel(f)?.source)
}
...
fn classify(op: &Operation) -> OpClass {
    use OpKind::*;
    match op.op_type {
        ArithAddf | ArithSubf | ArithMulf | ArithDivf | ArithMaximumf | ArithMaxf
        | ArithMinimumf | ArithMinf | ArithNegf | ArithAbsf | MathAbsf | MathExp | MathLog
        | MathSqrt | MathSin | MathCos | MathTanh | ArithExtf | ArithTruncf | LinalgAdd
        | LinalgMul | LinalgSub | LinalgBroadcast => OpClass::Map,
        LinalgReduce => OpClass::Reduce,
        LinalgMatmul => OpClass::Matmul,
        ...
        ArithConstant | TensorSplat | TensorEmpty | FuncReturn => OpClass::Plumbing,
        other if other.dialect() == Dialect::Ktdp => OpClass::Plumbing,
        _ => OpClass::Boundary,
    }
}
...
pub fn matmul_loop_schedule(ops: &[Operation]) -> HashMap<Ssa, MatmulLoopInfo> {
    let defs = def_map_all(ops);
    ...
    for op in ops {
        ...
        {
            sched.insert(info.out_ssa, info);
        }
    }
    sched
}
...
struct WeightKey {
    /// ⭐ THE WEIGHT'S HBM BYTE ADDRESS — the identity, because it is the only one that is stable
    /// ACROSS segments. This was the operand's root `Ssa`, which is local to the function being
    /// executed: a fused segment renumbers from 0, so the same name means different tensors in
    /// different segments and one tensor has different names in different programs. Everything
    /// that has to recognise a weight later — above all [`retain_resident_weights`], which decides
    /// what survives a `set_sources` — needs a name that outlives one function, and the resident
    /// sticks are allocated once per session so the address is exactly that.
    addr: i64,
    len: usize,
    fingerprint: u64,
    ...
    /// lm_head's 8 column tiles share `addr` but differ here) so they cache apart.
    col_off: i64,
...
fn weight_fingerprint(
    ...
    // pass to pass — skip it and key on the weight's HBM ADDRESS instead.
    //
    // ⛔ `WeightKey`'s `root` DOES NOT IDENTIFY A WEIGHT. It is an `Ssa` in the executed
    // function's own space, and a fused segment renumbers from 0 (`ktir-optimizer`'s
    // `fusion.rs` `Renamer`), so segment N's `%3` and segment N+1's `%3` are different
    // tensors. With the content sample skipped, `root + len + col_off` then collides for
    // any two same-shaped weights: k_proj and v_proj (both `[kv_dim, hidden]`) and the
    // SAME weight in every layer. MEASURED on llama-3.2-1b — every layer's `new_k` and
    // `new_v` came back byte-identical, and correct under `KTIR_TILE_DATAFLOW=1`, which
    // does not set this flag.
    //
    // ⭐ THE ADDRESS IS THE IDENTITY, and it is exactly what a resident weight has: the
    // sticks are allocated once per session, so two distinct weights are at two distinct
    // addresses by construction. That is strictly sharper than sampling content (which
    // only samples) and costs one hash rather than ~66 HBM reads.
    if TRUSTED_WEIGHTS.with(std::cell::Cell::get) {
        byte_addr.hash(&mut h);
        return h.finish();
    }
    if n == 0 {
        return h.finish();
...
pub static WEIGHT_CACHE_HITS: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);
/// Nanoseconds spent RESOLVING gemm operands (decode + upload / cache lookup) this forward, and
/// nanoseconds spent in the gemm itself. A forward is hundreds of small GEMMs, so the question is
/// whether the cost is the ARITHMETIC or the per-op marshalling around it.
#[cfg(metal)]
pub static GEMM_RESOLVE_NS: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
#[cfg(metal)]
pub static GEMM_COMPUTE_NS: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
/// [`GEMM_COMPUTE_NS`] split by the engine that ran it — GPU (`use_nax`) vs AMX
/// `cblas`. The counts alone (`matmul_loop_gpu`/`_amx`) can't say which engine owns
/// the wall time, and the two have completely different remedies.
#[cfg(metal)]
pub static GEMM_NAX_NS: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
#[cfg(metal)]
pub static GEMM_AMX_NS: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
/// Σ of each GEMM command buffer's own `GPUEndTime - GPUStartTime` — the time the
/// GPU was actually executing, excluding the commit/wait round trip. The gap between
/// this and [`GEMM_NAX_NS`] is what dispatch batching could recover.
#[cfg(metal)]
pub static GEMM_GPU_EXEC_NS: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
/// Σ of every offloaded GEMM's M and the count — the average tells you whether a DECODE step is
/// really running one row per GEMM or the whole prefill width.
#[cfg(metal)]
pub static GEMM_M_SUM: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
#[cfg(metal)]
pub static GEMM_COUNT: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(0);
...
pub static WEIGHT_CACHE_MISSES: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);
/// Cached buffers dropped because the tensor they hold was WRITTEN (KV cache, a re-set input).
/// These are correct drops: the bytes really did change, so the next read must re-decode.
#[cfg(metal)]
pub static WEIGHT_CACHE_DROP_MUTATED: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);
/// Cached buffers dropped because their HBM address matched no resident tensor. Unlike
/// [`WEIGHT_CACHE_DROP_MUTATED`] these are pure LOSS — an immutable weight thrown away and
/// re-decoded next step purely because we could not recognise it. Steady state should be 0.
#[cfg(metal)]
pub static WEIGHT_CACHE_DROP_UNRESOLVED: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);
...
///
/// `addr_tid` maps each resident tensor's HBM base address to its id; the caller owns that
/// allocation, so it passes the map in. An entry whose address is absent is dropped — safe,
/// since dropping only costs a re-decode — and counted in [`WEIGHT_CACHE_DROP_UNRESOLVED`],
/// which is the difference between "this weight legitimately changed" and "we failed to
/// recognise a weight we already had".
#[cfg(metal)]
pub fn retain_resident_weights(
    addr_tid: &HashMap<i64, u64>,
    ...
    WEIGHT_CACHE.with(|c| {
        c.borrow_mut()
            .retain(|key, _| match addr_tid.get(&key.addr) {
                None => {
                    WEIGHT_CACHE_DROP_UNRESOLVED.fetch_add(1, Relaxed);
                    false
                }
                Some(tid) => {
                    let mutated = forward_written.contains(tid) || just_set.contains(tid);
                    if mutated {
                        WEIGHT_CACHE_DROP_MUTATED.fetch_add(1, Relaxed);
                    }
                    !mutated
                }
            })
...
pub fn run_matmul_loop_gpu(
    ...
    let c = GEMM_ENGINE.with(|cell| -> Result<Vec<f32>, String> {
        ...
        GEMM_M_SUM.fetch_add(m as u64, std::sync::atomic::Ordering::Relaxed);
        GEMM_COUNT.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let _t_mm = std::time::Instant::now();
        let out = if use_nax {
            ...
            // Every m, including decode's m=1, goes to the tiled kernel. Routing m=1
            // to `gemv_unified` MEASURES SLOWER (GPU exec 104 vs 94 ms/token, ITL 200.8
            // vs 192): that kernel gives each thread one output column and a serial
            // K-long dependent FMA chain, which loses to this kernel's staged operand
            // reuse even though it carries 127 phantom rows.
        ...
        // AMX time is 80 synchronous `cblas` calls. Which of the two owns the 128ms
        // decides whether the lever is dispatch batching or a better CPU GEMM.
        let engine_ns = if use_nax { &GEMM_NAX_NS } else { &GEMM_AMX_NS };
        engine_ns.fetch_add(_mm_ns, std::sync::atomic::Ordering::Relaxed);
        Ok(out)
    })?;
    ...
    ctx.track_lx(info.out_ssa, bytes).map_err(|e| {
        format!(
            "{e} (GEMM out [{m}, {n}] k={k} n_off={} b_stride={})",
            info.n_off, info.b_stride
        )
    })?;
...
fn resolve_gemm_operand_unified_off(
    ...
    match ctx.get_value(root)? {
        ...
        crate::ir::Value::Tile(t) => {
            // ⛔ WIDEN STRAIGHT INTO THE UPLOAD BUFFER. `as_f32()` widens the WHOLE tile into a
            // fresh `Vec` (and for f16 storage `f16_units_to_f32` builds a byte `Vec` on the way),
            // which this then copied AGAIN into the unified buffer — three allocations and three
            // passes to move one activation. At the m=96 prefill rung the A operand is `[96, k]`
            // and there are 361 GEMMs a forward, which is where the 1.13 s of "operand resolve"
            // came from.
            let mut buf = UnifiedBuffer::new(&engine.device, n)?;
            if !t.write_f32_into(elem_off, buf.as_mut_slice()) {
                return Err(format!(
                    "metal: GEMM operand %{} resident tile has {} elems, need {} at row_off {row_off}",
                    root.0,
                    t.len(),
                    elem_off + n
                ));
        ...
        other => Err(format!(
            "metal: GEMM operand %{} is {other:?}, want tile/ptr",
            root.0
        )),
    }
}
...
fn resolve_gemm_weight_slice(
    ...
    let elem = match ctx.get_value(root)? {
        ...
        other => {
            return Err(format!(
                "metal: N-tiled GEMM weight %{} is {other:?}, want an HBM pointer",
                root.0
            ));
        }
    };
...
fn resolve_gemm_bt_operand(
    ...
    let elem = match ctx.get_value(root)? {
        ...
        other => {
            return Err(format!(
                "metal: transpose-B weight %{} is {other:?}, want an HBM pointer",
                root.0
            ));
        }
    };
    ...
    let key = WeightKey {
        ...
        // retention resolves the base against the resident allocation.
        addr: elem * bpe,
        len: count,
        fingerprint,
        col_off: n_off,
...
pub fn run_plain_matmul_gpu(
    ...
    let out_ssa = op.result.ok_or("metal: plain matmul has no result SSA")?;
    ...
    ctx.track_lx(out_ssa, bytes)
        .map_err(|e| format!("{e} (map-window out [{m}, {n}])"))?;
...
fn recognize_reduce_combiner(op: &Operation) -> Option<ReduceCombiner> {
    let name = match op.attr(AttrKey::ReduceFn) {
        Some(Attr::Op(k)) => *k,
        _ => op
            .regions
            .iter()
            .flat_map(|r| r.iter())
            .find(|o| o.op_type != OpKind::LinalgYield)
            .map(|o| o.op_type)?,
    };
    match name {
        OpKind::ArithAddf => Some(ReduceCombiner {
            is_max: false,
            identity: 0.0,
        }),
        OpKind::ArithMaximumf | OpKind::ArithMaxf => Some(ReduceCombiner {
            is_max: true,
...
pub fn run_reduce_gpu(op: &Operation, ctx: &mut crate::context::CoreContext) -> Result<(), String> {
    ...
    let dims = int_list_attr_vec(op, AttrKey::Dimensions).unwrap_or_default();
    if dims.as_slice() != [1] {
        return Err(format!(
    ...
    let out_ssa = op.result.ok_or("metal: reduce has no result SSA")?;
    ...
    if let Some(Attr::Ssas(outs)) = op.attr(AttrKey::OutsVar)
        && let Some(&outs_var) = outs.first()
    {
...
pub fn run_transpose_gpu(
    ...
    let perm = int_list_attr_vec(op, AttrKey::Permutation)
        .ok_or("metal: transpose missing permutation — interpreter")?;
    let out_ssa = op.result.ok_or("metal: transpose has no result SSA")?;
    ...
    let tile = crate::tile::Tile::compute(out, dtype, out_shape.clone());
    let bytes = tile.size_bytes() as i64;
    ctx.set_value(out_ssa, crate::ir::Value::Tile(tile));
    ctx.track_lx(out_ssa, bytes)
        .map_err(|e| format!("{e} (transpose out {out_shape:?})"))?;
...
fn expect_resident_tile(
    ...
    match ctx.get_value(v)? {
        ...
        other => Err(format!(
            "metal: {what} %{} is {other:?}, want a resident tile",
            v.0
        )),
    }
}

/// Result value -> defining op, recursively through regions. A matmul K-loop
/// references views/producers defined OUTSIDE the loop body and loads/slices
/// defined INSIDE it, so recognition needs a function-wide map.
fn def_map_all<'o>(ops: &'o [Operation]) -> HashMap<Ssa, &'o Operation<'o>> {
    let mut m = HashMap::new();
    fn rec<'a>(ops: &'a [Operation], m: &mut HashMap<Ssa, &'a Operation<'a>>) {
        for op in ops {
...
/// The MSL identifier a value's buffer is bound under. The buffer used to be
/// named by the SSA SPELLING with its `%` stripped; a value has no spelling, so
/// it is named by its identity.
fn buf_ident(v: Ssa) -> String {
    format!("v{}", v.0)
}
...
fn recognize_matmul_loop(
    ...
    let mut mms = body.iter().filter(|o| o.op_type == OpKind::LinalgMatmul);
    let mm = mms.next()?;
    if mms.next().is_some() {
        return None;
    }
    ...
    let mm_res = mm.result?;
    // Single loop-carried accumulator.
    let iter_args = match forop.attr(AttrKey::IterArgs) {
        Some(Attr::Ssas(v)) if v.len() == 1 => v,
        _ => return None,
    };
    let acc = iter_args[0];
    // The accumulate: addf(acc, matmul_result) (either operand order).
    let addf = body.iter().find(|o| {
        o.op_type == OpKind::ArithAddf && o.operands.contains(&acc) && o.operands.contains(&mm_res)
    })?;
    let addf_res = addf.result?;
    // The loop yields the accumulate.
    let yld = body.iter().find(|o| o.op_type == OpKind::ScfYield)?;
    if yld.operands.first().copied() != Some(addf_res) {
        return None;
    }
...
fn matmul_a_row_offset(name: Ssa, defs: &HashMap<Ssa, &Operation>) -> Option<i64> {
    let d = defs.get(&name)?;
    if d.op_type != OpKind::KtdpLoad {
        return None;
    }
    let tile = d.operands.first()?;
    let tile_op = defs.get(tile)?;
    ...
    let cd = defs.get(idx)?;
    if cd.op_type != OpKind::ArithConstant {
        return None;
    }
    match cd.attr(AttrKey::Value) {
...
fn matmul_b_axis_offset(name: Ssa, defs: &HashMap<Ssa, &Operation>, last: bool) -> Option<i64> {
    let d = defs.get(&name)?;
    if d.op_type != OpKind::KtdpLoad {
...
fn matmul_operand_full(name: Ssa, defs: &HashMap<Ssa, &Operation>) -> Option<(Ssa, Vec<i64>)> {
    ...
    match d.op_type {
        ...
        OpKind::KtdpLoad => {
            let tile = d.operands.first()?;
            let view = defs.get(tile)?.operands.first()?;
...
pub struct MapRegionKernel {
    ...
    /// the reduce sum), or any value defined outside the window — the VALUES the
    /// runtime resolves from the value table.
    pub live_ins: Vec<Ssa>,
    /// The single window result consumed outside the window.
    pub live_out: Ssa,
...
fn emit_map_region_kernel_with(
    ...
    window: &[usize],
    defs: &HashMap<Ssa, &Operation>,
    uses: &HashMap<Ssa, HashSet<usize>>,
) -> Result<MapRegionKernel, String> {
    if window.is_empty() {
    ...
    let mut live_outs: Vec<Ssa> = Vec::new();
    for &i in window {
        let Some(name) = ops[i].result else {
            continue;
        };
    ...
    let in_window: HashSet<Ssa> = window.iter().filter_map(|&i| ops[i].result).collect();
    let live_out_name = live_outs[0];
    let root = *defs
        .get(&live_out_name)
    ...
    let kname = format!("map_region_{}", buf_ident(live_out_name));
    buffers.push(BufferBinding {
        name: buf_ident(live_out_name),
        is_output: true,
        dtype: DType::F32,
    });
...
fn build_uses(ops: &[Operation]) -> HashMap<Ssa, HashSet<usize>> {
    fn note(op: &Operation, top_idx: usize, uses: &mut HashMap<Ssa, HashSet<usize>>) {
        for &operand in op.operands {
            uses.entry(operand).or_default().insert(top_idx);
        }
        for (_, attr) in op.attributes {
            if let Attr::Ssas(xs) = attr {
                for &x in *xs {
                    uses.entry(x).or_default().insert(top_idx);
                }
            }
        }
        for region in op.regions {
    ...
    let mut uses: HashMap<Ssa, HashSet<usize>> = HashMap::new();
    for (i, op) in ops.iter().enumerate() {
        note(op, i, &mut uses);
    }
    uses
...
fn lower_map_value(
    ...
    let in_win = in_window.contains(&name);
    match defs.get(&name) {
        ...
        Some(d) if d.op_type == OpKind::ArithConstant => constant_literal(d),
        Some(d) if d.op_type == OpKind::TensorSplat => {
        ...
        Some(d) if d.op_type == OpKind::LinalgBroadcast => {
            ...
            let input_in_win = in_window.contains(&input);
            let input_is_load = defs
                .get(&input)
                .is_some_and(|x| x.op_type == OpKind::KtdpLoad);
            ...
            let input_scalar = shape_attr_vec(defs.get(&input).copied())
            ...
            if input_in_win && !input_is_load {
                ...
                if input_scalar {
                    ...
                    Err(format!(
                        "metal: rank-reducing broadcast of in-window value %{} \
        ...
        // A non-constant SCALAR value is a `Value::Scalar` at runtime — it can't be
        // bound as a tile buffer and we can't fold it. Fail the window.
        ...
        Some(d) if !is_tensor_valued(d) => Err(format!(
            "metal: map window needs scalar value %{} (def {:?}) — not a resident tile; \
...
fn try_fold_scalar(name: Ssa, defs: &HashMap<Ssa, &Operation>) -> Option<String> {
    let d = defs.get(&name)?;
    match d.op_type {
        OpKind::ArithConstant => constant_literal(d).ok(),
        OpKind::TensorSplat => try_fold_scalar(*d.operands.first()?, defs),
        _ => None,
    }
}
...
fn lower_map_broadcast(
    ...
    let in_shape =
        shape_attr_vec(defs.get(&input).copied()).ok_or("metal: broadcast input has no shape")?;
    let out_shape = shape_attr_vec(Some(op)).ok_or("metal: broadcast has no output shape")?;
    let mut dims = int_list_attr_vec(op, AttrKey::Dimensions).unwrap_or_default();
    dims.sort_unstable();
    ...
    // Register the input buffer (dedup).
    if !live_ins.contains(&input) {
        live_ins.push(input);
    }
    let buf = buf_ident(input);
    Ok(format!(
        "{buf}[{}]",
...
pub fn run_map_region_gpu(
    ...
    for &name in &mrk.live_ins {
        match ctx.get_value(name)? {
            ...
            other => {
                return Err(format!(
                    "metal: map-region live-in %{} is {other:?}, expected a resident tile or scalar",
                    name.0
                ));
            }
        }
...
pub fn map_fusion_plan(ops: &[Operation]) -> MapRegionPlan {
    ...
    let try_flush = |window: &mut Vec<usize>,
        ...
        let emitted = emit_map_region_kernel_with(ops, &w, &defs, &uses);
        if let Err(e) = &emitted
            && std::env::var_os("KTIR_REWRITE_VERBOSE").is_some()
        {
            // A window that never fuses is indistinguishable from a slow one from outside.
            eprintln!("  [map-window] {} ops REFUSED: {e}", w.len());
        }
        if let Ok(mrk) = emitted {
    ...
    for (i, op) in ops.iter().enumerate() {
        ...
        // ⭐ A `ktdp.store` ENDS A CHAIN, so it ends the window.
        //
        // `emit_map_region_kernel_with` requires EXACTLY ONE live-out, and a store is what makes a
        // value live-out. Classified as plumbing, a store does not break the window, so N
        // independent chains — the row/row-block passes an elementwise or silu node emits, one
        // store each — pile into ONE window with N live-outs and emit refuses every time. MEASURED:
        // `map_region=0` on every granite forward, i.e. the whole fused-map GPU path was compiled
        // and never reached, leaving ~19 ms per layer of interpreted f32 elementwise.
        //
        // Flushing BEFORE the store is the right order: the fused kernel produces the live-out, then
        // the store (left on the interpreter, hence `continue`) writes it.
...
pub fn emit_kernel(f: &IRFunction) -> Result<MslKernel, String> {
    let defs = def_map(f);
    ...
    for b in collect_input_buffers(compute, &defs) {
        let name = buf_ident(b);
        if !buffers.iter().any(|x| x.name == name) {
    ...
    buffers.push(BufferBinding {
        name: buf_ident(out_buf),
        is_output: true,
        dtype,
    });

    let source = render_kernel(f.name, &buffers, &expr);
    Ok(MslKernel {
        source,
...
/// `result value -> defining op`.
fn def_map<'o>(f: &'o IRFunction<'o>) -> HashMap<Ssa, &'o Operation<'o>> {
    let mut m = HashMap::new();
...
/// -> `construct_memory_view` -> the `%ptr` argument. Returns that argument.
fn trace_buffer(name: Ssa, defs: &HashMap<Ssa, &Operation>) -> Option<Ssa> {
    let mut cur = name;
    // Walk defining ops until we hit a value with no def (a function argument).
    for _ in 0..16 {
        let Some(op) = defs.get(&cur) else {
...
fn collect_input_buffers(compute: &Operation, defs: &HashMap<Ssa, &Operation>) -> Vec<Ssa> {
    let mut out = Vec::new();
    collect_bufs(compute, defs, &mut out, 0);
    out
}

fn collect_bufs(op: &Operation, defs: &HashMap<Ssa, &Operation>, out: &mut Vec<Ssa>, depth: usize) {
    if depth > MAX_FUSE_DEPTH {
        return;
    }
    for &operand in op.operands {
        match defs.get(&operand) {
            ...
            Some(d) if d.op_type == OpKind::KtdpLoad => {
...
fn buffer_dtype(buf: Ssa, f: &IRFunction) -> DType {
    for op in f.operations {
        if op.op_type == OpKind::KtdpConstructMemoryView
            && op.operands.first().copied() == Some(buf)
            && let Some(crate::ir::Attr::Dtype(dt)) = op.attr(AttrKey::Dtype)
        {
            return *dt;
        }
    }
...
fn msl_type(dt: DType) -> &'static str {
    match dt {
        DType::F16 => "half",
        // ⛔ MSL HAS NO e4m3 TYPE. An fp8 operand is widened on the way in, so the kernel it feeds
        // computes in `half` — the narrowing back to one byte is the STORE's, not the kernel's.
        DType::Fp8E4m3 => "half",
        DType::F32 => "float",
        DType::I32 => "int",
        DType::I64 => "long",
...
fn lower_compute(op: &Operation, defs: &HashMap<Ssa, &Operation>) -> Result<String, String> {
...
fn lower_value(name: Ssa, defs: &HashMap<Ssa, &Operation>, depth: usize) -> Result<String, String> {
    if depth > MAX_FUSE_DEPTH {
        return Err("metal: fused expression exceeds max depth".into());
    }
    match defs.get(&name) {
        None => Err(format!("metal: operand %{} has no defining op", name.0)),
        Some(d) if d.op_type == OpKind::KtdpLoad => {
            let buf = trace_buffer(name, defs)
                .ok_or_else(|| format!("metal: operand %{} is not a loaded buffer", name.0))?;
            Ok(format!("{}[gid]", buf_ident(buf)))
        }
        ...
        Some(d) if d.op_type == OpKind::LinalgBroadcast => lower_broadcast(d, defs),
...
fn lower_broadcast(op: &Operation, defs: &HashMap<Ssa, &Operation>) -> Result<String, String> {
    let input = *op
        .operands
        .first()
        .ok_or("metal: linalg.broadcast missing ins operand")?;
    let buf = buf_ident(trace_buffer(input, defs).ok_or(
...
fn shape_attr_vec(op: Option<&Operation>) -> Option<Vec<i64>> {
    match op?.attr(AttrKey::Shape) {
        Some(crate::ir::Attr::IntList(v)) => Some(v.to_vec()),
        _ => None,
    }
}

/// Read an `IntList` attribute as an `i64` vector.
fn int_list_attr_vec(op: &Operation, key: AttrKey) -> Option<Vec<i64>> {
    match op.attr(key) {
        Some(crate::ir::Attr::IntList(v)) => Some(v.to_vec()),
        _ => None,
    }
...
fn compose_compute_expr(
    ...
    match op.op_type {
        OpKind::ArithAddf => binop("+", resolve),
        OpKind::ArithSubf => binop("-", resolve),
        OpKind::ArithMulf => binop("*", resolve),
        OpKind::ArithDivf => binop("/", resolve),
        OpKind::ArithMaximumf | OpKind::ArithMaxf => Ok(format!(
            "max({}, {})",
            operand(0, resolve)?,
            operand(1, resolve)?
        )),
        OpKind::ArithMinimumf | OpKind::ArithMinf => Ok(format!(
            "min({}, {})",
            operand(0, resolve)?,
            operand(1, resolve)?
        )),
        OpKind::ArithNegf => Ok(format!("-{}", operand(0, resolve)?)),
        OpKind::ArithAbsf | OpKind::MathAbsf => unary("abs", resolve),
        OpKind::MathExp => unary("exp", resolve),
        OpKind::MathLog => unary("log", resolve),
        OpKind::MathSqrt => unary("sqrt", resolve),
        OpKind::MathSin => unary("sin", resolve),
        OpKind::MathCos => unary("cos", resolve),
        OpKind::MathTanh => unary("tanh", resolve),
        OpKind::LinalgAdd => binop("+", resolve),
        OpKind::LinalgMul => binop("*", resolve),
        OpKind::LinalgSub => binop("-", resolve),
        // A scalar constant folds into the expression as an MSL literal.
        OpKind::ArithConstant => constant_literal(op),
        ...
        OpKind::TensorSplat => operand(0, resolve),
        ...
        OpKind::ArithExtf => Ok(format!("float({})", operand(0, resolve)?)),
        OpKind::ArithTruncf => Ok(format!("half({})", operand(0, resolve)?)),
        other => Err(format!(
...
impl Epilogue {
    ...
    pub fn from_binary_op(op_type: OpKind) -> Option<Epilogue> {
        Some(match op_type {
            OpKind::LinalgAdd | OpKind::ArithAddf => Epilogue::ADD,
            OpKind::LinalgMul | OpKind::ArithMulf => Epilogue::MUL,
            OpKind::LinalgSub | OpKind::ArithSubf => Epilogue::SUB,
            OpKind::LinalgMax | OpKind::ArithMaximumf | OpKind::ArithMaxf => Epilogue::MAX,
            OpKind::LinalgMin | OpKind::ArithMinimumf | OpKind::ArithMinf => Epilogue::MIN,
            _ => return None,
        })
    }

    /// Map a unary activation KTIR op to its epilogue, or `None`.
    pub fn from_unary_op(op_type: OpKind) -> Option<Epilogue> {
        Some(match op_type {
            OpKind::MathTanh => Epilogue::TANH,
            OpKind::MathExp => Epilogue::EXP,
            _ => return None,
        })
    }
}
...
///
/// Field order matters: `mtl` is released before `alloc` frees the memory. Both are
/// [`ManuallyDrop`](std::mem::ManuallyDrop) so [`Drop`] can hand the pair to
/// [`BUFFER_POOL`] intact instead of unmapping and freeing it.
#[cfg(metal)]
pub struct UnifiedBuffer {
    mtl: std::mem::ManuallyDrop<MtlBuf>,
    alloc: std::mem::ManuallyDrop<AlignedAlloc>,
    len: usize,
...
/// The reusable half of a [`UnifiedBuffer`]: the page-aligned host block and the
/// Metal buffer mapped over *that exact pointer and length*. The two are only ever
/// recycled together — pairing a pooled `mtl` with a different `alloc` would hand
/// the GPU a stale mapping.
#[cfg(metal)]
struct BufferParts {
    mtl: MtlBuf,
    alloc: AlignedAlloc,
...
#[cfg(metal)]
thread_local! {
    /// Recycled [`BufferParts`], keyed by backing-allocation size in bytes.
    ///
    /// `newBufferWithBytesNoCopy` is not a cheap call: the driver has to wire the
    /// pages and map them into the GPU address space, and releasing the buffer
    /// unmaps them again. A decode step issues one GEMM output buffer per matmul
    /// plus one per freshly-uploaded activation operand — hundreds of map/unmap
    /// pairs per token, all at shapes that are IDENTICAL from one token to the
    /// next. Pooling by byte size turns every one of those after the first token
    /// into a pop.
    ///
    /// Entries are dropped normally once [`POOL_MAX_BYTES`] is reached, so the pool
    /// cannot grow without bound on a workload whose shapes keep changing.
    static BUFFER_POOL: std::cell::RefCell<HashMap<usize, Vec<BufferParts>>> =
        std::cell::RefCell::new(HashMap::new());
    /// Total bytes currently held by [`BUFFER_POOL`], against [`POOL_MAX_BYTES`].
    static POOL_BYTES: std::cell::Cell<usize> = const { std::cell::Cell::new(0) };
}

/// Ceiling on retained [`BUFFER_POOL`] bytes. Decode's working set is the per-token
/// activation and GEMM-output buffers — megabytes, not gigabytes (the multi-GB model
/// weights live in `WEIGHT_CACHE` and are never released to this pool) — so this is
/// far above steady-state need and exists only to bound a shape-churning workload.
#[cfg(metal)]
...
impl Drop for UnifiedBuffer {
    fn drop(&mut self) {
        // SAFETY: both fields are ManuallyDrop, taken exactly once here, and never
        // read again (this is the only Drop impl and `self` dies immediately after).
        let parts = unsafe {
            BufferParts {
                mtl: std::mem::ManuallyDrop::take(&mut self.mtl),
                alloc: std::mem::ManuallyDrop::take(&mut self.alloc),
            }
        };
        let bytes = parts.alloc.layout.size();
        // `try_with`, not `with`: a buffer can outlive the pool itself during thread
        // teardown, once the TLS slots are destroyed. There is nothing to recycle into
        // then, and `with` would panic on the AccessError.
        let room = POOL_BYTES
            .try_with(|p| {
                let fits = p.get() + bytes <= POOL_MAX_BYTES;
                if fits {
                    p.set(p.get() + bytes);
                }
                fits
            ...
            .unwrap_or(false);
        if room {
            // On the teardown path `try_with` drops the closure without calling it,
            // which frees the captured `parts` — the same release we'd do by hand.
            let _ = BUFFER_POOL.try_with(|c| c.borrow_mut().entry(bytes).or_default().push(parts));
        }
        // Otherwise `parts` falls out of scope and frees mtl then alloc, in that order.
    }
}
...
impl UnifiedBuffer {
    ...
    fn new_sized(
        ...
        // A recycled pair skips both the host allocation and — the expensive half —
        // the driver's page-wiring in `newBufferWithBytesNoCopy`. Zero it so a pooled
        // buffer is byte-for-byte indistinguishable from a fresh `alloc_zeroed` one;
        // a memset over these sizes is orders of magnitude below the mapping it saves.
        let recycled = POOL_BYTES.with(|p| {
            BUFFER_POOL.with(|c| {
                let parts = c.borrow_mut().get_mut(&bytes)?.pop()?;
                p.set(p.get().saturating_sub(bytes));
                Some(parts)
            })
        });
        if let Some(parts) = recycled {
            // SAFETY: `ptr` is the head of this pair's own `bytes`-long allocation.
            unsafe { std::ptr::write_bytes(parts.alloc.ptr, 0, bytes) };
            return Ok(Self {
                mtl: std::mem::ManuallyDrop::new(parts.mtl),
                alloc: std::mem::ManuallyDrop::new(parts.alloc),
                len,
                elem_bytes,
            });
        }
        ...
        Ok(Self {
            mtl: std::mem::ManuallyDrop::new(mtl),
            alloc: std::mem::ManuallyDrop::new(AlignedAlloc { ptr, layout }),
            len,
            elem_bytes,
        })
    }
...
/// The four `KTIR_SGS_N=1` (TG_N=64) pipelines, by B layout and B element type — the
/// same (transpose_b, f16-B) matrix the full-width pipelines cover. Dispatched at m=1
/// with a grid width of `n / NARROW_BLOCK_N` and [`NARROW_THREADS`] per threadgroup.
#[cfg(metal)]
struct NarrowN {
    plain: objc2::rc::Retained<
        objc2::runtime::ProtocolObject<dyn objc2_metal::MTLComputePipelineState>,
    >,
    bt: objc2::rc::Retained<
        objc2::runtime::ProtocolObject<dyn objc2_metal::MTLComputePipelineState>,
    >,
    plain_f16b: objc2::rc::Retained<
        objc2::runtime::ProtocolObject<dyn objc2_metal::MTLComputePipelineState>,
    >,
    bt_f16b: objc2::rc::Retained<
        objc2::runtime::ProtocolObject<dyn objc2_metal::MTLComputePipelineState>,
    >,
}

/// TG_N of the narrow-N variant: `SG_N(64) * KTIR_SGS_N(1)`.
#[cfg(metal)]
const NARROW_BLOCK_N: usize = 64;
/// TG_THREADS of the narrow-N variant: `SGS_M(4) * SGS_N(1) * 32`.
#[cfg(metal)]
const NARROW_THREADS: usize = 128;
...
pub struct NaxGemm {
    ...
    gemv_pipeline_bt: objc2::rc::Retained<
        objc2::runtime::ProtocolObject<dyn objc2_metal::MTLComputePipelineState>,
    >,
    /// f16-B GEMV pipelines (`KTIR_B_F16=1`), plain and transpose-B. Required for the
    /// decode fast path to read the same resident `half` weight buffer the matmul
    /// kernels do; without them GEMV would need an f32 B and stream twice the bytes.
    gemv_pipeline_f16b: objc2::rc::Retained<
        objc2::runtime::ProtocolObject<dyn objc2_metal::MTLComputePipelineState>,
    >,
    gemv_pipeline_bt_f16b: objc2::rc::Retained<
        objc2::runtime::ProtocolObject<dyn objc2_metal::MTLComputePipelineState>,
    >,
    /// Narrow-N (TG_N=64) NAX variants for the m=1 decode grid — `None` off NAX.
    narrow_n: Option<NarrowN>,
...
impl NaxGemm {
    ...
    fn compile(force_nax: Option<bool>) -> Result<Self, String> {
        ...
        // NARROW-N (decode) variant: `KTIR_SGS_N=1` -> TG_N=64, so the m=1 grid gets 4x
        // the threadgroups at the same total thread count. Built from source only —
        // the AOT stem encodes (tb, sm, f16b) and has no field for SGS_N, so an `aot`
        // hit here would silently return the TG_N=256 pipeline and be dispatched over a
        // 4x-too-wide grid: every threadgroup would recompute the same columns.
        let build_narrow = |tb: u32,
                            f16b: u32|
         -> Result<
            objc2::rc::Retained<
                objc2::runtime::ProtocolObject<dyn objc2_metal::MTLComputePipelineState>,
        ...
        > {
            let full = format!(
                "#define KTIR_TRANSPOSE_B {tb}\n#define KTIR_B_F16 {f16b}\n\
                 #define KTIR_SGS_N 1\n{src}"
            );
            let library = device
                .newLibraryWithSource_options_error(&NSString::from_str(&full), Some(&opts))
                .map_err(|e| format!("metal: {kname} narrow-N compile failed (tb={tb}): {e:?}"))?;
            let function = library
                .newFunctionWithName(&NSString::from_str(kname))
                .ok_or_else(|| format!("metal: kernel {kname} not found"))?;
            device
                .newComputePipelineStateWithFunction_error(&function)
                .map_err(|e| format!("metal: {kname} narrow-N pipeline failed (tb={tb}): {e:?}"))
        };
        let narrow_n = if is_nax {
            Some(NarrowN {
                plain: build_narrow(0, 0)?,
                bt: build_narrow(1, 0)?,
                plain_f16b: build_narrow(0, 1)?,
                bt_f16b: build_narrow(1, 1)?,
            })
        } else {
            None
        };
        ...
        // f16-B GEMV: the decode (m=1) weight operand is the SAME resident f16 buffer
        // the matmul kernels read, so without these variants routing m=1 to GEMV would
        // silently demand an f32 B and double the streamed bytes.
        // Built from source, NOT via `aot`: the gemv AOT stem is `nax_gemv__tb{tb}`,
        // which does not encode f16b, so an AOT hit would hand back the f32-B pipeline
    ...
    pub fn matmul_unified(
        ...
        // DECODE (m=1): prefer the narrow-N variant. The default TG_N=256 leaves the
        // grid at [N/256, 1] — 32 threadgroups for an N=8192 FFN, 8 for an N=2048
        // projection on a 10-core GPU. TG_N=64 gives 4x the threadgroups over the same
        // threads. Only at m=1: at larger m the grid already has `m/128` rows of
        // threadgroups and the wider tile's B reuse is worth more.
        let narrow = (m == 1 && !small)
            .then_some(self.narrow_n.as_ref())
            .flatten();
        ...
        let pipe = match (narrow, transpose_b, b_f16) {
            (Some(nn), true, true) => &nn.bt_f16b,
            (Some(nn), true, false) => &nn.bt,
            (Some(nn), false, true) => &nn.plain_f16b,
            (Some(nn), false, false) => &nn.plain,
            _ => match (small, transpose_b, b_f16) {
                (true, true, true) => self
                    .pipeline_sm_bt_f16b
                    .as_ref()
        ...
        let (blk_n, blk_m, threads) = match (narrow.is_some(), small) {
            (true, _) => (NARROW_BLOCK_N, self.block_m, NARROW_THREADS),
