...
#[derive(Clone, Debug)]
pub struct Binding {
    /// Which of the function's arguments this binds.
    pub arg: Ssa,
...
pub struct FusedSegment<'a> {
    pub func: IRFunction<'a>,
    /// ⭐ WHICH TENSOR EACH ARGUMENT POINTS AT, in `func.arguments` order. The
    /// executor used to recover this by PARSING the argument's name (`%t{id}_ptr`
    /// -> `id`); a value has no spelling to parse, and [`fuse_program`] already
    /// knows the mapping, so it travels with the segment.
    pub args: Vec<(Ssa, u64)>,
...
pub fn is_attention_node(func: &IRFunction<'_>) -> bool {
    ...
    fn scan(ops: &[Operation<'_>], has_transpose: &mut bool, has_softmax_reduce: &mut bool) {
        for op in ops {
            if op.op_type == OpKind::LinalgTranspose {
                *has_transpose = true;
            }
            ...
            if op.op_type == OpKind::LinalgReduce
                && (matches!(
                    op.attr(AttrKey::ReduceFn),
                    Some(Attr::Op(OpKind::ArithMaximumf))
                ) || region_has_op(op.regions, OpKind::ArithMaximumf))
            {
...
pub fn is_decode_attention_node(func: &IRFunction<'_>) -> bool {
    let (gx, gy, gz) = func.grid;
    if gx * gy * gz != 1 {
        return false;
    }
...
pub fn node_partition_dims(func: &IRFunction, node: &NodeSpec) -> PartitionDims {
    ...
    let mut arg_tensor: HashMap<Ssa, u64> = HashMap::new();
    for b in &node.bindings {
        arg_tensor.insert(b.arg, b.tensor);
    }
    let mut pid: Option<Ssa> = None;
    let mut view_arg: HashMap<Ssa, Ssa> = HashMap::new();
    // access-tile SSA -> (parent view SSA, [index SSA per dim]).
    let mut acc: HashMap<Ssa, (Ssa, Vec<Ssa>)> = HashMap::new();
    ...
    fn walk(
        ...
        acc: &mut HashMap<Ssa, (Ssa, Vec<Ssa>)>,
        ...
        for op in ops {
            match op.op_type {
                ...
                OpKind::KtdpConstructMemoryView => {
                    if let (Some(r), Some(&ptr)) = (op.result, op.operands.first()) {
                        view_arg.insert(r, ptr);
                    }
                }
                OpKind::KtdpConstructAccessTile => {
                    if let (Some(r), Some(&view)) = (op.result, op.operands.first()) {
                        let idx: Vec<Ssa> = op.operands[1..].to_vec();
                        acc.insert(r, (view, idx));
                    }
                }
                OpKind::KtdpLoad | OpKind::KtdpStore => {
                    let is_store = op.op_type == OpKind::KtdpStore;
                    if let Some((_, (view, idx))) =
                        op.operands.iter().find_map(|o| acc.get_key_value(o))
...
fn region_has_op(regions: &[&[Operation<'_>]], op_type: OpKind) -> bool {
    regions.iter().any(|rg| {
        rg.iter()
...
pub fn plan_segments_budgeted<'a>(
    a: &'a Arena,
    module: &IRModule<'a>,
    spec: &ProgramSpec,
    lx_budget: usize,
...
fn build_fused_segment<'a>(
    ...
    let (mut func, arg_tensors) = fuse_program(a, module, &seg_spec)?;
    ...
    func.grid = (1, 1, 1);
    // Classify the fused function's surviving pointer args by direction against the boundary sets:
    // a boundary OUTPUT iff its tensor is in `seg_results`, a boundary INPUT iff in `seg_sources`.
    // Anything else is an INTERNAL SCRATCH arg (an intra-segment edge fusion could NOT forward as
    // SSA — resident HBM the fused fn writes then reads in its own body; the runner zero-inits it).
    let mut outputs: HashSet<u64> = HashSet::new();
    let mut inputs: HashSet<u64> = HashSet::new();
    for (_, id) in &arg_tensors {
        let id = *id;
...
pub fn fuse_program<'a>(
    a: &'a Arena,
    module: &IRModule<'a>,
    spec: &ProgramSpec,
) -> Result<(IRFunction<'a>, Vec<(Ssa, u64)>), String> {
    ...
    let mut produced: HashMap<u64, (Ssa, Vec<i64>)> = HashMap::new();
    ...
    let mut needed_args: Vec<(u64, Ssa)> = Vec::new();
    // ⭐ ONE VALUE PER TENSOR. The fused function's pointer argument for a tensor used to be the
    // NAME `%t{id}_ptr`, so every node had to spell the same identity the same way. It is minted
    // once here and aliased into each node's space instead.
    let mut canon_arg: HashMap<u64, Ssa> = HashMap::new();
    let mut rename = Renamer::default();
    let mut have_arg: HashSet<u64> = HashSet::new();
    let mut body: Vec<Operation> = Vec::new();

    for (ni, node) in spec.nodes.iter().enumerate() {
        let an = &analyses[ni];
        let arg_to_tensor: HashMap<Ssa, &Binding> =
            node.bindings.iter().map(|b| (b.arg, b)).collect();
        ...
        let mut forwarded_args: HashSet<Ssa> = HashSet::new();
        let mut loads_by_arg: HashMap<Ssa, Vec<&LoadChain>> = HashMap::new();
        for ld in &an.loads {
            loads_by_arg.entry(ld.arg).or_default().push(ld);
        }
        ...
        for st in &an.stores {
            ...
            {
                forwarded_args.insert(st.arg);
                produced.insert(b.tensor, (rename.get(ni, st.stored), st.view_shape.clone()));
            }
        }
        ...
        let mut drop_results: HashSet<Ssa> = HashSet::new();
        // ktdp.store ops whose tile operand is in here are dropped.
        let mut drop_store_tiles: HashSet<Ssa> = HashSet::new();
        // load result SSA -> the extract_slice that replaces it (tiled forward).
        let mut slice_at_load: HashMap<Ssa, SliceForward> = HashMap::new();
        ...
        for ld in &an.loads {
            ...
            let Some(b) = arg_to_tensor.get(&ld.arg) else {
                continue;
            };
            ...
            if ld.whole_tensor {
                rename.alias(ni, ld.loaded, *val);
                drop_results.insert(ld.loaded);
            } else {
        ...
        for b in &node.bindings {
            ...
            let canon = *canon_arg.entry(b.tensor).or_insert_with(|| rename.mint());
            rename.alias(ni, b.arg, canon);
    ...
    // ⭐ THE ARG->TENSOR MAPPING TRAVELS WITH THE FUNCTION. It was recovered by parsing the arg's
    // NAME (`%t{id}_ptr` -> `id`), which only worked while the name encoded the id; a value has no
    // spelling to parse, so the mapping the builder already knows is returned instead.
    let arg_tensors: Vec<(Ssa, u64)> = needed_args.iter().map(|(t, v)| (*v, *t)).collect();
    let args: Vec<(Ssa, IrType<'a>)> = needed_args
        .into_iter()
        .map(|(_, v)| (v, IrType::Index))
        .collect();
    body.push(Operation::new(a, None, OpKind::FuncReturn, &[]));
    Ok((
        IRFunction {
            name: a.str(String::from("fused")),
            arguments: a.args(args),
            operations: a.ops(body),
            grid: spec
                .nodes
                .first()
                .map(|n| {
...
impl SliceForward {
    fn build<'a>(&self, a: &'a Arena, ni: usize, rename: &mut Renamer) -> Operation<'a> {
        let res = rename.get(ni, self.loaded);
        let offsets: Vec<Ssa> = self.offsets.iter().map(|o| rename.get(ni, *o)).collect();
        let sizes: Vec<i64> = self.sizes.clone();
        let strides: Vec<i64> = self.sizes.iter().map(|_| 1).collect();
        Operation::new(a, Some(res), OpKind::TensorExtractSlice, &[self.source])
            .with_attr(a, AttrKey::SliceOffsets, Attr::Ssas(a.ssa(offsets)))
            .with_attr(a, AttrKey::SliceSizes, Attr::IntList(a.ints(sizes)))
            .with_attr(a, AttrKey::SliceStrides, Attr::IntList(a.ints(strides)))
    }
}
...
struct LoadChain {
    arg: Ssa,
    loaded: Ssa,
    /// The view the access tile is built on (dropped when the arg is forwarded).
    #[allow(dead_code)]
    view: Ssa,
    /// The access tile — identifies the tile op to drop and the load to rewrite.
    tile: Ssa,
    whole_tensor: bool,
...
struct Analysis {
    ...
    /// view -> (the arg pointer it interprets, view shape).
...
fn shape_attr_of(op: &Operation<'_>) -> Vec<i64> {
...
fn collect_views_tiles(ops: &[Operation<'_>], a: &mut Analysis) {
    for op in ops {
        match op.op_type {
            OpKind::KtdpConstructMemoryView => {
                if let (Some(res), Some(&arg)) = (op.result, op.operands.first()) {
                    a.views.insert(res, (arg, shape_attr_of(op)));
                }
            }
            OpKind::KtdpConstructAccessTile => {
                if let (Some(res), Some(&view)) = (op.result, op.operands.first()) {
                    a.tiles.insert(
                        ...
                        TileInfo {
                            ...
                            has_order: op.attr(AttrKey::CoordinateOrder).is_some(),
                        },
                    );
                }
            }
...
fn base_map_is_identity(tile_op: &Operation<'_>) -> bool {
    match tile_op.attr(AttrKey::BaseMap) {
...
/// Mints the fused function's SSA values.
///
/// Fusing N nodes into one body puts N independently-numbered value spaces in one scope, so each
/// node's values need fresh identities. That used to be a NAME PREFIX (`%l` -> `%n0_l`), which is
/// why an unrenamed declaration could still parse and then fail to resolve at run time. Minting an
/// id per `(node, value)` pair makes the collision impossible instead of detectable.
#[derive(Default)]
struct Renamer {
    mapped: HashMap<(usize, Ssa), Ssa>,
    next: u32,
}

impl Renamer {
    /// This node's identity for `v`, minting one on first sight.
    fn get(&mut self, ni: usize, v: Ssa) -> Ssa {
        if let Some(&m) = self.mapped.get(&(ni, v)) {
            return m;
        }
        let fresh = Ssa(self.next);
        self.next += 1;
        self.mapped.insert((ni, v), fresh);
        fresh
    }

    /// Bind `v` in node `ni` to an identity another node already minted — how a forwarded
    /// intermediate becomes one value instead of a copy.
    fn alias(&mut self, ni: usize, v: Ssa, to: Ssa) {
        self.mapped.insert((ni, v), to);
    }

    /// A fresh identity belonging to no node — the fused function's own values.
    fn mint(&mut self) -> Ssa {
        let fresh = Ssa(self.next);
        self.next += 1;
        fresh
    }
...
fn rename_attrs<'a>(
    ...
    // ⭐ EVERY SSA-VALUED ATTRIBUTE IS RENAMED, WHATEVER ITS KEY. This walked a hardcoded list —
    // `iter_var`, `iter_args`, `result_names`, `sizes_dyn`, `outs_var` — and a region's
    // block-argument declaration was not on it, so fusion rewrote every USE of `%l` to `%n0_l`
    // while the declaration stayed `%l`: `undefined SSA value: %n0_l`. Typing those attributes as
    // [`Attr::Ssas`] leaves no list to leave an entry out of.
    //
    // `outs_var` is why this matters beyond that one bug: it names a `linalg.reduce`'s init
    // accumulator, so leaving it unrenamed gives every node's reduce ONE shared slot — node N folds
    // node N-1's stale partial instead of its own freshly-splat 0.0, which grew the RMSNorm
    // sum-of-squares across layers and moved the e2e golden by ~30 logits.
    let attrs: Vec<(AttrKey, Attr<'a>)> = op
        .attributes
        .iter()
        .map(|(k, v)| match v {
            Attr::Ssas(xs) => (
                *k,
                Attr::Ssas(a.ssa(xs.iter().map(|s| rename.get(ni, *s)).collect())),
            ),
            other => (*k, other.clone()),
        })
        .collect();
...
fn emit_ops<'a>(
    ...
    ni: usize,
    rename: &mut Renamer,
    drop_results: &HashSet<Ssa>,
    drop_store_tiles: &HashSet<Ssa>,
    slice_at_load: &HashMap<Ssa, SliceForward>,
) -> Vec<Operation<'a>> {
    let mut out = Vec::new();
    for op in ops {
        if op.op_type == OpKind::FuncReturn {
            continue;
        }
        if let Some(r) = op.result
        ...
        if op.op_type == OpKind::KtdpLoad
            && let Some(r) = op.result
            && let Some(sf) = slice_at_load.get(&r)
        {
            out.push(sf.build(a, ni, rename));
            continue;
        }
        out.push(Operation {
            result: op.result.map(|r| rename.get(ni, r)),
            op_type: op.op_type,
            operands: a.ssa(op.operands.iter().map(|o| rename.get(ni, *o)).collect()),
            attributes: rename_attrs(a, op, ni, rename),
