...
%outtensor, %bnA, %bnB, %bnA2, %bnB2, %bias, %resadd = ddl.tensor(%slice_layout_output, %stick_layout_output, %global_layout_output, [%type_fp16]) : index, index, index, index, index, index, index
...
%relu6_intermediate = ddl.internal_tensor(%outtensor, [%type_fp16]) : index
...
%relu6_op = ddl.operation_bind([], [%outtensor], [%outtensor], [%relu6_intermediate]) {opFuncName="relu6fwd", required=false}
...
ddl.dataflow {
  ...
  ddl.datastage_constraint(%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {values = ["1","2","4"]}
  ...
  ddl.loop (%d_datastage, %b_datastage, %in, %out, %krd#0, %krd#1, %krd#2, %ki, %kj){label="chunk_loop"} {  
    ...
    ddl.loop (%b_datastage, %bottom_datastage, %out) {} {
      ddl.loop (%b_datastage, %blk_datastage, %in, %ki, %kj) {label="blkload_loop_in"} {
        ...
        %cond_last_accum_loops = ddl.condition_and(%cond_last_chunk_in_loop, %cond_last_chunk_ki_loop,
                                                    %cond_last_chunk_kj_loop, %cond_last_blkload_in_loop,
    ...
    %cond_last_chunk_loop_in_ki_kj = ddl.condition_and(%cond_last_chunk_in_loop, %cond_last_chunk_ki_loop, %cond_last_chunk_kj_loop)
    %is_relu_or_leakyrelu = ddl.condition_or(%relu_op, %leakyrelu_op, %relu6_op)

    %is_any_aux_op = ddl.condition_or(%bn_op, %bn2_op, %stradd_op, %bias_op, %is_relu_or_leakyrelu)
    %is_no_aux_op = ddl.condition_not(%is_any_aux_op)

    %is_bn_not_last = ddl.condition_or(%bn2_op, %stradd_op, %bias_op, %is_relu_or_leakyrelu)

    %is_bn2_not_first = ddl.condition_or(%bn_op)
    %is_bn2_not_last = ddl.condition_or(%stradd_op, %bias_op, %is_relu_or_leakyrelu)

    %is_bias_not_first = ddl.condition_or(%bn_op, %bn2_op, %stradd_op)
    %is_bias_not_last = ddl.condition_or(%is_relu_or_leakyrelu)

    %is_stradd_not_first = ddl.condition_or(%bn_op, %bn2_op)
    %is_stradd_not_last = ddl.condition_or(%bias_op, %is_relu_or_leakyrelu)

    %is_relu_not_first = ddl.condition_or(%bn_op, %bn2_op, %stradd_op, %bias_op)
    %is_relu_first = ddl.condition_not(%is_relu_not_first)

    ddl.if (%is_any_aux_op) {
      ddl.if (%cond_last_chunk_loop_in_ki_kj) {
        ...
        ddl.loop (%b_datastage, %bottom_datastage, %out) {} {
          ...
          ddl.loop (%b_datastage, %interleave_datastage_aux, %krd#0, %krd#1, %krd#2) {} {
            // load output to lx-sfp-fifo
            %src_out_lxsfp = ddl.unit(%outtensor, %outtensor_lx_allocation) {unit="lxlu", data_connect="lxsu_output"}
            %dst_out_lxsfp = ddl.unit(%outtensor) {unit="sfp", data_connect="lxsfp_outtensor"} 
            ...
            // bn
            ddl.if (%bn_op) {
              ddl.loop (%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {} {
                %sfp_bn_src00_lxlu = ddl.unit(%outtensor) {unit="lxlu", data_connect="lxsfp_outtensor"}
            ...
            ddl.if (%bn2_op) {
              ddl.loop (%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {} {
                %sfp_bn2_src00_lxlu = ddl.unit(%outtensor) {unit="lxlu", data_connect="lxsfp_outtensor"}
                ...
                ddl.if(%is_bn2_not_first) {
                  ...
                  ddl.if(%is_bn2_not_last) {
                    // bn: lrf * lxlu + lrf -> lrf
                    ddl.compute([%sfp_bn2_src00_lxlu, %sfp_bn2_src01, %sfp_bn2_src02], [%sfp_bn2_dst00_lrf]) {computetype="FMA16", unit="sfp"}
                  } else {
                    // bn: lrf * lxlu + lrf -> lxsu
                    ddl.compute([%sfp_bn2_src00_lxlu, %sfp_bn2_src01, %sfp_bn2_src02], [%sfp_bn2_dst00_lxsu]) {computetype="FMA16", unit="sfp"}                                        
                  }
                }
              }
            }
            ...
            ddl.if (%stradd_op) {
              ...
              ddl.loop (%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {} {
                %sfp_stradd_src00_lxlu = ddl.unit(%outtensor) {unit="lxlu", data_connect="lxsfp_outtensor"}
                ...
                ddl.if(%is_stradd_not_first) {
                  ddl.if(%is_stradd_not_last) {
                    // stridedadd: lrf + pe -> lrf
            ...
            ddl.if (%bias_op) {
              ddl.loop (%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {} {
                %sfp_biasadd_src00_lxlu = ddl.unit(%outtensor) {unit="lxlu", data_connect="lxsfp_outtensor"}
            ...
            ddl.if (%relu_op) {
              ddl.loop (%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {} {
                %sfp_relu_src00_lxlu = ddl.unit(%outtensor) {unit="lxlu", data_connect="lxsfp_outtensor"}
            ...
            ddl.if (%leakyrelu_op) {
              ...
              ddl.if(%is_relu_first) {
                ddl.loop (%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {} {
                  %sfp_relu_src00_lxlu = ddl.unit(%outtensor) {unit="lxlu", data_connect="lxsfp_outtensor"}
                  // store partial sum: lxlu -> lrf
                  ddl.compute([%sfp_relu_src00_lxlu, %one_const], [%sfp_partialsum_lrf]) {computetype="FMUL", unit="sfp"}
                }
              }
              ddl.loop (%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {} {
                // leaky val: lrf -> lrf
            ...
            ddl.if (%relu6_op) {
              ...
              ddl.loop (%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {} {
                ...
                ddl.if(%is_relu_not_first) {
                  ...
                  // relu: lxlu -> lrf
                  ddl.compute([%sfp_relu_src00_lxlu, %zero_const], [%sfp_relu_lrf]) {computetype="FMAX", unit="sfp"}
                }
              }
              ddl.loop (%interleave_datastage_aux, %bottom_datastage, %krd#0, %krd#1, %krd#2) {} {
                %sfp_relu_dst00_lxsu = ddl.unit(%outtensor) {unit="lxsu", data_connect="sfp_lxsu_outtensor"}
