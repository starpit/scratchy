...
%slice_layout_input = ddl.layout(%in) {is_order_fixed=false}  // only allow IN channel in the stick until we implement strided load in L0LU
...
%slice_layout_input_scale = ddl.layout(%in, %wrd#0) {is_order_fixed=false}
%stick_layout_input_scale = ddl.layout(%in) {is_order_fixed=true} 
%global_layout_input_scale = ddl.layout(%in, %wrd#0, %wrd#1, %wrd#2, %wrd#3, %nrd#0, %nrd#1, %wrdpd, %wrdd#1) {}
...
%type_bf16 = ddl.type {data_type="BFLOAT16"}
...
%type_fp4 = ddl.type {data_type="SEN121_FP4"}
%type_int24 = ddl.type {data_type="SENINT24"}
%type_053fp8 = ddl.type {data_type="SEN053_FP8"}   // Type for MX scale tensor
%type_080fp8 = ddl.type {data_type="SEN080_FP8"}   // Type for MX scale tensor
...
%inptensor_fp16 = ddl.tensor(%slice_layout_input_16bit, %stick_layout_input, %global_layout_input, [%type_bf16]) : index
%kertensor_fp16 = ddl.tensor(%slice_layout_kernel_16bit, %stick_layout_kernel, %global_layout_kernel, [%type_bf16]) : index
%inptensor_fp8 = ddl.tensor(%slice_layout_input, %stick_layout_input, %global_layout_input, [%type_fp8]) : index
%inptensor_fp4 = ddl.tensor(%slice_layout_input, %stick_layout_input, %global_layout_input, [%type_fp4]) : index
%inptensor_scale = ddl.tensor(%slice_layout_input_scale, %stick_layout_input_scale, %global_layout_input_scale, [%type_053fp8, %type_080fp8]) : index
...
%kertensor_fp4 = ddl.tensor(%slice_layout_kernel, %stick_layout_kernel, %global_layout_kernel, [%type_fp4]) : index
%kertensor_scale = ddl.tensor(%slice_layout_kernel, %stick_layout_kernel, %global_layout_kernel, [%type_053fp8, %type_080fp8]) : index
...
%pesum = ddl.internal_tensor(%outtensor, [%type_fp32]) : index
%outtensor_fp32 = ddl.internal_tensor(%outtensor, [%type_fp32]) : index

// operation..
%bmm_int8_mbkg3_op = ddl.operation_bind([%type_int8], [%inptensor_int8, %kertensor_int8], [%outtensor], [%ptsum_int, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulint8mbkg3", required=false}
%bmm_int8_op = ddl.operation_bind([%type_int8], [%inptensor_int8, %kertensor_int8], [%outtensor], [%ptsum_int, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulint8", required=false}
%bmm_fp16_op = ddl.operation_bind([%type_bf16], [%inptensor_fp16, %kertensor_fp16], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="batchmatmul", required=false}
%bmm_fp8_op = ddl.operation_bind([%type_fp8], [%inptensor_fp8, %kertensor_fp8], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulfp8", required=false}
%bmm_fp8_mb_op = ddl.operation_bind([%type_fp8], [%inptensor_fp8, %kertensor_fp8], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulfp8mb", required=false}
%bmm_int4_op = ddl.operation_bind([%type_int4], [%inptensor_int4, %kertensor_int4], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulint4", required=false}
%mm_int8_op = ddl.operation_bind([%type_int8], [%inptensor_int8, %kertensor_int8], [%outtensor], [%ptsum_int, %pesum, %outtensor_fp32]) {opFuncName="matmulint8", required=false}
%mm_fp16_op = ddl.operation_bind([%type_bf16], [%inptensor_fp16, %kertensor_fp16], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="matmul", required=false}
%mm_fp8_op = ddl.operation_bind([%type_fp8], [%inptensor_fp8, %kertensor_fp8], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="matmulfp8", required=false}
%mm_int4_op = ddl.operation_bind([%type_int4], [%inptensor_int4, %kertensor_int4], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="matmulint4", required=false}
%bmmxrf_int8_op = ddl.operation_bind([%type_int8], [%inptensor_int8, %kertensor_int8], [%outtensor], [%ptsum_int, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulxrfint8", required=false}
%bmmxrf_fp16_op = ddl.operation_bind([%type_bf16], [%inptensor_fp16, %kertensor_fp16], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulxrf", required=false}
%bmmxrf_fp8_op = ddl.operation_bind([%type_fp8], [%inptensor_fp8, %kertensor_fp8], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulxrffp8", required=false}
%bmmxrf_int4_op = ddl.operation_bind([%type_int4], [%inptensor_int4, %kertensor_int4], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulxrfint4", required=false}
%bmm_mxfp8_op = ddl.operation_bind([%type_fp8], [%inptensor_fp8, %inptensor_scale, %kertensor_fp8, %kertensor_scale], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="batchmatmulmxfp8", required=false}
%bmm_mxfp4_op = ddl.operation_bind([%type_fp4], [%inptensor_fp4, %inptensor_scale, %kertensor_fp4, %kertensor_scale], [%outtensor], [%ptsum_fp, %pesum, %outtensor_fp32]) {opFuncName="scaledgroupmatmulfp4", required=false}

%fp16_bmm = ddl.condition_or(%bmm_fp16_op, %mm_fp16_op, %bmmxrf_fp16_op) 
%fp8_bmm = ddl.condition_or(%bmm_fp8_op, %bmm_fp8_mb_op, %mm_fp8_op, %bmmxrf_fp8_op, %bmm_mxfp8_op) 
...
%fp_bmm = ddl.condition_or(%fp16_bmm, %fp8_bmm, %bmm_mxfp4_op)
%int_bmm = ddl.condition_or(%int8_bmm, %int4_bmm)

%mx_bmm = ddl.condition_or(%bmm_mxfp8_op, %bmm_mxfp4_op)
...
ddl.constraint(%inptensor_int8) {property = "slice", dim_idx = 0, cmp = "equal", value = 16}
ddl.constraint(%inptensor_int4) {property = "slice", dim_idx = 0, cmp = "equal", value = 32}
...
ddl.constraint(%bmm_int8_mbkg3_op, %bmm_int8_op, %bmm_fp16_op, %bmm_fp8_op, %bmm_fp8_mb_op, %bmm_int4_op, 
                ...
                %bmm_mxfp8_op, %bmm_mxfp4_op) {min_num_valid = 1, max_num_valid = 1}
ddl.constraint(%bmm_int8_mbkg3_op, %bmm_int8_op, %bmm_fp16_op, %bmm_fp8_op, %bmm_fp8_mb_op, %bmm_int4_op,
                %bmmxrf_int8_op, %bmmxrf_fp16_op, %bmmxrf_fp8_op, %bmmxrf_int4_op, 
                %mm_int8_op, %mm_fp16_op, %mm_fp8_op, %mm_int4_op,
...
%inptensor = ddl.alias_one_tensor_of(%inptensor_int8, %inptensor_fp16, %inptensor_fp8, %inptensor_int4,  %inptensor_fp4)
%kertensor = ddl.alias_one_tensor_of(%kertensor_int8, %kertensor_fp16, %kertensor_fp8, %kertensor_int4, %kertensor_fp4)
...
%inptensor_scale_lx_allocation = ddl.get_external_data_transfer_allocation (%inptensor_scale) {memory="lx", data_connect="l3_lx_input_scale"}
...
%kertensor_scale_lx_allocation = ddl.get_external_data_transfer_allocation (%kertensor_scale) {memory="lx", data_connect="l3_lx_kernel_scale"}
%kertensor_scale_xrf_ext_allocation = ddl.get_external_data_transfer_allocation (%kertensor_scale) {memory="ptxrf", data_connect="xrf_kernel_scale"}
...
ddl.dataflow {
  ...
  %blk_rows_iter_datastage = ddl.datastage {strategy="maximize"}  // IN channel in 1 slice of activations in fp16 (before quantization)
  %blk_rows_iter_datastage_quirk1 = ddl.datastage {strategy="maximize"}  // how many sticks of fp16 activations are fused during quantization
  %blk_rows_iter_datastage_quirk2 = ddl.datastage {strategy="maximize"}  // 2 slices get combined in one row because of sen1p5 row size
  ...
  %mx_datastage = ddl.datastage {strategy="minimize"}
  %accum2_datastage = ddl.datastage {strategy="maximize"}
  ...
  ddl.datastage_constraint(%accum_in_stick_datastage, %inptensor, %in) {values=["0.25","0.5"]}  // 1/2 (one row) or 2 sticks to cover accum2_datastage
  ddl.if(%fp_bmm) {
    ddl.if(%fp16_bmm) {
      ddl.datastage_constraint(%blk_rows_iter_datastage, %kertensor, %in) {values=["16"]}
      ddl.datastage_constraint(%blk_rows_iter_datastage_quirk1, %blk_rows_iter_datastage, %in) {values=["1"]}
      ddl.datastage_constraint(%blk_rows_iter_datastage_quirk2, %blk_rows_iter_datastage_quirk1, %in) {values=["1"]}
    } 
    ddl.if(%fp8_bmm) {
      ddl.datastage_constraint(%blk_rows_iter_datastage, %kertensor, %in) {values=["4"]}
      ddl.datastage_constraint(%blk_rows_iter_datastage_quirk1, %blk_rows_iter_datastage, %in) {values=["2"]}
      ddl.datastage_constraint(%blk_rows_iter_datastage_quirk2, %blk_rows_iter_datastage_quirk1, %in) {values=["2"]}
    }
    ddl.if(%bmm_mxfp4_op) {
      ddl.datastage_constraint(%blk_rows_iter_datastage, %kertensor, %in) {values=["4"]}
      ddl.datastage_constraint(%blk_rows_iter_datastage_quirk1, %blk_rows_iter_datastage, %in) {values=["2"]}
    ...
    ddl.datastage_constraint(%bottom_datastage, %kertensor, %in) {values=["8"]}
  }
  ddl.if(%int_bmm) {
    ddl.if(%int8_bmm) {
      ddl.datastage_constraint(%blk_rows_iter_datastage, %kertensor, %in) {values=["4"]}
  ...
  ddl.datastage_constraint(%accum2_datastage, %bottom_datastage, %in) {max="2"}
  ...
  ddl.loop (%d_datastage, %b_datastage, %in, %out, %nrd#0, %nrd#1, %wrd#0, %wrd#1, %wrd#2, %wrd#3){label="chunk_loop"} {  
    ...
    ddl.loop (%b_datastage, %bottom_datastage, %out, %nrd#0, %nrd#1) {} {
      ...
      ddl.loop (%b_datastage, %blk_datastage, %in) {label="blkload_loop"} {
        ...
        %kertensor_scale_xrf_allocation = ddl.allocate(%kertensor_scale, %kertensor_scale_xrf_ext_allocation) {memory="ptxrf"}

        ddl.if (%kertensor_lx_allocation) {
          ddl.loop (%blk_datastage, %blk_rows_iter_datastage_quirk2, %in) {} {
            ddl.loop (%blk_rows_iter_datastage_quirk2, %blk_rows_iter_datastage_quirk1, %in) {} {
              ddl.loop (%blk_rows_iter_datastage_quirk1, %blk_rows_iter_datastage, %in) {} {
        ...
        // block load scale for Mx-computation from LX into XRF
        //    # of scale elements = # of value elements / mx-block size
        ddl.if (%mx_bmm) {
          ddl.loop (%blk_datastage, %blk_rows_iter_datastage_quirk2, %in) {} {
            ddl.loop (%blk_rows_iter_datastage_quirk2, %blk_rows_iter_datastage_quirk1, %in) {} {
              %src_ker_scale_lxpt = ddl.unit(%kertensor_scale, %kertensor_scale_lx_allocation) {unit="lxlu", data_connect="l3_lx_kernel_scale"}
              %dst_ker_scale_lxpt = ddl.unit(%kertensor_scale, %kertensor_scale_xrf_allocation) {unit="pt", data_connect="xrf_kernel_scale"}
              ddl.data_transfer(%src_ker_scale_lxpt, [%dst_ker_scale_lxpt]) {}
            }
          }
        }
        ...
        ddl.loop (%b_datastage, %interleave_datastage, %wrd#0, %wrd#1, %wrd#2, %wrd#3) {} {
          ...
          ddl.if(%mx_bmm) {
            ddl.force_innermost_dimensions(%inptensor_l0_allocation, %mx_datastage, %wrd#0, %in)
          }

          ddl.loop (%blk_datastage, %mx_datastage, %in) {} {            
            ddl.loop (%interleave_datastage, %mx_datastage, %wrd#0) {} {
          ...
          // load input scale to lx-l0
          %inptensor_scale_l0_allocation = ddl.allocate(%inptensor_scale) {memory="l0scale", num_buffers=-1:si64}
          ddl.if(%mx_bmm) {
            // A separate implicit_sync is not needed for the MX-scale allocation
            ddl.loop (%blk_datastage, %mx_datastage, %in) {} {            
              ddl.loop (%interleave_datastage, %mx_datastage, %wrd#0) {} {
                %src_inp_scale_lxl0 = ddl.unit(%inptensor_scale, %inptensor_scale_lx_allocation) {unit="lxlu", data_connect="l3_lx_input_scale"}
                %dst_inp_scale_lxl0 = ddl.unit(%inptensor_scale, %inptensor_scale_l0_allocation) {unit="l0su", data_connect="l0_input_scale"} 
                ddl.data_transfer(%src_inp_scale_lxl0, [%dst_inp_scale_lxl0]) {}
              }
            }
          }

          // load output to lx-pe-fifo
          %outtensor_pe_allocation = ddl.allocate(%outtensor_fp32) {memory="pelrf"}
          %pe_out_lrf = ddl.unit(%outtensor_fp32, %outtensor_pe_allocation) {unit="pe", data_connect="lxpe_output"}
          ddl.if(%psum_end) {
            ddl.if(%cond_not_first_inloop) {
            ...
            ddl.loop (%accum_in_stick_datastage, %accum2_datastage, %in) {label="accumstickaccum2_loop"} {
              ...
              ddl.loop (%accum2_datastage, %bottom_datastage, %in) {label="accum2bottom_loop"} {
                ...
                ddl.loop (%interleave_datastage, %bottom_datastage, %wrd#0, %wrd#1, %wrd#2, %wrd#3) {} {
                  ...
                  // For Mx matmul, the scale will be automatically loaded from L0 by the hardware.
                  // No explicit transfer of scale from L0 to PT is needed. 
                  ...
                  ddl.if(%cond_first_accum2bottomloop) {
                    ddl.if(%cond_last_accum2bottomloop) {
                      // flavor1: 0 + store to south
                      ddl.compute([%pt_src00, %pt_src01, %zero_const], [%pt_dst00_south]) {computetype="MACC", unit="ptrow0"}
                      // flavor2: north + store to south
                      ddl.compute([%pt_src00, %pt_src01, %pt_src02_north], [%pt_dst00_south]) {computetype="MACC", unit="ptrow1-3"}
                    } else {
                      ...
                      ddl.compute([%pt_src00, %pt_src01, %pt_src02_north], [%pt_dst00_arf]) {computetype="MACC", unit="ptrow1-3"}
                    }
                  } else {
                    ddl.if(%cond_last_accum2bottomloop) {
                  ...
                  // pt-pe-ptsum
                  ddl.if(%cond_last_accum2bottomloop) {
                    %src_ptsum_ptpe = ddl.unit(%ptsum) {unit="ptrow3", data_connect="arf_ptsum"}
              ...
              ddl.loop (%interleave_datastage, %bottom_datastage, %wrd#0, %wrd#1, %wrd#2, %wrd#3) {} {
                ...
                ddl.if(%cond_first_blkaccum2loop) {
                  ...
                  ddl.compute([%pe_fma_src00, %one_const, %zero_const], [%pe_fma_lrf]) {computetype="FMA32", unit="pe"}
                } else {
                  // chunk accumulation in PE: lrf + ptsum => lrf
                  ddl.compute([%pe_fma_src00, %one_const, %pe_fma_lrf], [%pe_fma_lrf]) {computetype="FMA32", unit="pe"}                                        
                }
              }
            }
          }
          ...
          ddl.if(%psum_end) {
            ddl.if(%cond_not_first_inloop) {
              ddl.loop (%interleave_datastage, %bottom_datastage, %wrd#0, %wrd#1, %wrd#2, %wrd#3) {} {
                ...
                ddl.compute([%pe_out_lrf, %one_const, %pe_fma_lrf], [%dst_out_pesfp]) {computetype="FMA32", unit="pe"}
              }
            // pe-sfp-output
            } else {
