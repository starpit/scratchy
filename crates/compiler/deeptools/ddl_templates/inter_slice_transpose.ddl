         ...
         ddl.loop (%b_datastage, %l0subchunk_datastage, %d#0, %d#1, %d#2,%d#3, %d#4, %d#5, %asdin, %asdout){label="subchunk_loop"} {
            ...
            // ddl.loop(%l0subchunk_datastage, %bottom_datastage,%asdout){}{
                //ddl.loop(%l0subchunk_datastage, %bottom_datastage,%asdout#0){}{
                    ...
                    %src_inp_l0fifo = ddl.unit(%inptensor, %inptensor_l0_allocation) {unit="l0lu", data_connect="l0_input"}
                    %dst_inp_l0fifo = ddl.unit(%inptensor) {unit="pt", data_connect="pt_fifo"} 
                    %src_out_compute = ddl.unit(%outtensor, %outtensor_arf_allocation) {unit="pt", data_connect="compute_out"} 
                    ddl.data_transfer(%src_inp_l0fifo, [%dst_inp_l0fifo]) {}
                    %dst_inp_fifo = ddl.unit(%inptensor) {unit="l0lu", data_connect="pt_fifo"} 
                    // ddl.compute([%dst_inp_fifo, %one_const, %zero_const], [%src_out_compute]) {computetype="MACC", unit="pt"}
                    ddl.compute([%zero_const, %one_const, %dst_inp_fifo], [%src_out_compute]) {computetype="MACC", unit="pt"}
                //}
            // }
