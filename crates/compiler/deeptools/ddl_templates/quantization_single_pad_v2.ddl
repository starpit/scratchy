...
module {
    ...
    // Tensor
    %input_tensor, %dummy_tensor_alamax, %dummy_tensor_alamin, %scale_tensor, %shift_tensor = 
    ...
    // Op
    %csqint8_op = ddl.operation_bind([], [%input_tensor,  %scale_tensor, %shift_tensor], [%output_int8_tensor_padded], [%inter_int8_tensor]) {opFuncName="csqint8v2", required=false}
    %csqint8mb_op = ddl.operation_bind([], [%input_tensor, %scale_tensor, %shift_tensor], [%output_int8_tensor], [%inter_int8_tensor]) {opFuncName="csqint8mbv2", required=false}
    %qfp8_op = ddl.operation_bind([], [%input_tensor], [%output_fp8_tensor]) {opFuncName="qfp8", required=false}

    // Constraints
    ddl.constraint(){min_num_cores = 1}
    ddl.constraint(%output_int8_tensor, %output_int8_tensor_padded, %output_fp8_tensor) {property = "slice", dim_idx = 0, cmp = "equal", value = 8}
    ddl.constraint(%csqint8_op, %csqint8mb_op, %qfp8_op) {min_num_valid = 1, max_num_valid = 1}
    ...
    // can use in dataflow 
    %output_tensor = ddl.alias_one_tensor_of(%output_int8_tensor, %output_int8_tensor_padded, %output_fp8_tensor)
    ...
    %allocate_handler_scale_lx = ddl.get_external_data_transfer_allocation(%scale_tensor) {memory="lx", data_connect="l3_lx_scale"}
    %allocate_handler_shift_lx = ddl.get_external_data_transfer_allocation(%shift_tensor) {memory="lx", data_connect="l3_lx_shift"}
            ...
            ddl.loop(%b_datastage, %bottom_datastage, %outer_dim#0, %outer_dim#1, %outer_dim#2, %outer_dim#3, %stick_dim) {} {
                ...
                ddl.if (%csqint8_op) {
                    ddl.parametric_loop (%z_front, %input_tensor) {} {
                        ...
                        ddl.compute([%pe_zero_lrf, %zero_const, %zero_const], [%pe_dst00]) {computetype="FMA16", unit="pe"}
                ...
                %scale_pe_allocation_tensor = ddl.allocate(%scale_tensor) {memory="pelrf"}
                %shift_pe_allocation_tensor = ddl.allocate(%shift_tensor) {memory="pelrf"}
                // LX->PE transfer
                %src_lxpe = ddl.unit(%input_tensor, %allocate_handler_input_lx) {unit="lxlu", data_connect="l3_lx_input"}
                %src_scale_tensor = ddl.unit(%scale_tensor, %allocate_handler_scale_lx) {unit="lxlu", data_connect= "l3_lx_scale"}
                %src_shift_tensor = ddl.unit(%shift_tensor, %allocate_handler_shift_lx) {unit="lxlu", data_connect= "l3_lx_shift"}
                    ddl.if (%csq_ops) {
                        %dst_lxpe = ddl.unit(%input_tensor) {unit="pe", data_connect="lx_pe_input"}
                        %pe_scale_tensor = ddl.unit(%scale_tensor, %scale_pe_allocation_tensor) {unit="pe", data_connect="scale_pe_lrf_tensor"}
                        %pe_shift_tensor = ddl.unit(%shift_tensor, %shift_pe_allocation_tensor) {unit="pe", data_connect="shift_pe_lrf_tensor"}
                        ddl.data_transfer(%src_scale_tensor, [%pe_scale_tensor]) {}
                        ddl.data_transfer(%src_shift_tensor, [%pe_shift_tensor]) {}
                        ...
                        %pe_dst00 = ddl.unit(%inter_int8_tensor) {unit="sfp", data_connect="pe_output"}
                        ddl.compute([%pe_src00, %pe_scale_tensor, %pe_shift_tensor], [%pe_dst00]) {computetype="FMA16", unit="pe"}
                        %src_out_pesfp = ddl.unit(%inter_int8_tensor) {unit="pe", data_connect="pe_output"}
                        %dst_out_pesfp = ddl.unit(%inter_int8_tensor) {unit="sfp", data_connect="sfp_input"}
                        ddl.data_transfer(%src_out_pesfp, [%dst_out_pesfp]) {}
                    }
