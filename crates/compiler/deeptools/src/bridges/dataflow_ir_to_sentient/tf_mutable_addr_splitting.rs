...
pub fn sort_data_based_on_weight(mas_data: &mut [MasData]) {
    // `return a.weight_ > b.weight_` — descending, so `Reverse` on the key.
    mas_data.sort_by_key(|a| std::cmp::Reverse(a.weight));
}
...
/// `d0 * s0`.
fn first_scaling_of_dim(expr: &AffineExpr, dim: u32) -> Option<&AffineExpr> {
...
fn time_bound_as_num_iters(bound: TimeBound) -> i64 {
    match bound {
        TimeBound::Steps(steps) => i64::try_from(steps).unwrap_or(i64::MAX),
