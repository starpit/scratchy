...
fn mint_constants<A: Arch>(
    ...
    for value in want {
        if let std::collections::hash_map::Entry::Vacant(e) = consts.minted.entry(value) {
            let fresh = vals.mint();
            e.insert(fresh);
            preamble.push(SenOp::Sentient(sen::Op::ScalarConstant {
                value,
...
fn rectangle(set: &crate::islands::dataflow_ir::ty::IntegerSet) -> Vec<u64> {
    ...
    for c in &set.constraints {
        ...
        if let AffineExpr::Add(lhs, rhs) = &c.expr
            && let (AffineExpr::Mul(inner, factor), AffineExpr::Const(bound)) =
                (lhs.as_ref(), rhs.as_ref())
                && let (AffineExpr::Dim(dim), AffineExpr::Const(-1)) =
