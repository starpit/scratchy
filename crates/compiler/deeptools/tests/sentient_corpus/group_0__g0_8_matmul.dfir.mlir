func.func @dataflowProgram() attributes {grid = [1]} {
  ...
  dataflow.program_unit %16 : {
    ...
    %22 = arith.constant 2101248 : index
    %23 = dataflow.get_logical_memory_view %0, %22 {layout_map = affine_map<(d0, d1) -> (d0 * 512 + d1)>} : index, index, memref<2048x512xf16>
    %24 = arith.constant 2048 : index
    %25 = dataflow.get_logical_memory_view %1, %24 {layout_map = affine_map<(d0, d1) -> (d0 * 512 + d1)>} : index, index, memref<2048x512xf16>
    ...
    {
      agen.yield
    } : memref<2048x512xf16>, memref<2048x512xf16>
    %27 = arith.constant 2625536 : index
    %28 = dataflow.get_logical_memory_view %0, %27 {layout_map = affine_map<(d0, d1) -> (d0 * 512 + d1)>} : index, index, memref<1x512xf16>
    %29 = arith.constant 1050624 : index
    %30 = dataflow.get_logical_memory_view %1, %29 {layout_map = affine_map<(d0, d1) -> (d0 * 512 + d1)>} : index, index, memref<1x512xf16>
  ...
  dataflow.program_unit %2 : {
    ...
    %35 = dataflow.get_logical_memory_view %1, %34 {layout_map = affine_map<(d0, d1) -> (d0 * 512 + d1)>} : index, index, memref<2048x512xf16>
    %36 = arith.constant 1050624 : index
    %37 = dataflow.get_logical_memory_view %1, %36 {layout_map = affine_map<(d0, d1) -> (d0 * 512 + d1)>} : index, index, memref<1x512xf16>
    ...
    %39 = agen.vector_load %35[0, 0] {load_order = affine_map<(d0, d1) -> (d0, d1)>, load_set = affine_set<(d0, d1) : (d0 == 0, d1 >= 0, -d1 + 63 >= 0)>} : memref<2048x512xf16>, vector<64xf16>
    dataflow.send %13, %39 : vector<64xf16>
    %40 = agen.vector_load %37[0, 0] {load_order = affine_map<(d0, d1) -> (d0, d1)>, load_set = affine_set<(d0, d1) : (d0 == 0, d1 >= 0, -d1 + 63 >= 0)>} : memref<1x512xf16>, vector<64xf16>
  ...
  dataflow.program_unit %15 : {
    ...
    %45 = dataflow.get_logical_memory_view %1, %44 {layout_map = affine_map<(d0, d1) -> (d0 * 512 + d1)>} : index, index, memref<1x512xf16>
    %46 = dataflow.receive %13 : vector<64xf16>
    agen.vector_store %46, %45[0, 0] {store_order = affine_map<(d0, d1) -> (d0, d1)>, store_set = affine_set<(d0, d1) : (d0 == 0, d1 >= 0, -d1 + 63 >= 0)>} : memref<1x512xf16>, vector<64xf16>
  }
  return
}
