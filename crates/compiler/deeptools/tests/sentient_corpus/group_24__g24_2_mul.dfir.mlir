func.func @dataflowProgram() attributes {grid = [1]} {
  ...
  dataflow.program_unit %6 : {
    %7 = arith.constant 196608 : index
    %8 = dataflow.get_logical_memory_view %0, %7 {layout_map = affine_map<(d0, d1) -> (d0 * 2048 + d1)>} : index, index, memref<1x2048xf16>
    %9 = arith.constant 0 : index
    %10 = dataflow.get_logical_memory_view %1, %9 {layout_map = affine_map<(d0, d1) -> (d0 * 2048 + d1)>} : index, index, memref<1x2048xf16>
  ...
  dataflow.program_unit %4 : {
    ...
    %13 = dataflow.get_logical_memory_view %1, %12 {layout_map = affine_map<(d0, d1) -> (d0 * 2048 + d1)>} : index, index, memref<1x2048xf16>
    %14 = agen.vector_load %13[0, 0] {load_order = affine_map<(d0, d1) -> (d0, d1)>, load_set = affine_set<(d0, d1) : (d0 == 0, d1 >= 0, -d1 + 63 >= 0)>} : memref<1x2048xf16>, vector<64xf16>
  ...
  dataflow.program_unit %2 {precision = "fp16"} : {
    ...
    %17 = vectorchain.binary %15, %16 {binary_op = #vectorchain<binary_operator mul>, op_specific_map = affine_map<(d0) -> (d0)>} : vector<64xf16>, vector<64xf16>, vector<64xf16>
    dataflow.send %5, %17 : vector<64xf16>
  }
  dataflow.program_unit %5 : {
    %18 = arith.constant 196608 : index
    %19 = dataflow.get_logical_memory_view %1, %18 {layout_map = affine_map<(d0, d1) -> (d0 * 2048 + d1)>} : index, index, memref<1x2048xf16>
    %20 = dataflow.receive %2 : vector<64xf16>
    agen.vector_store %20, %19[0, 0] {store_order = affine_map<(d0, d1) -> (d0, d1)>, store_set = affine_set<(d0, d1) : (d0 == 0, d1 >= 0, -d1 + 63 >= 0)>} : memref<1x2048xf16>, vector<64xf16>
  }
  return
}
